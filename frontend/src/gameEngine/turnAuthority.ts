...
export function turnRefusal(input: TurnAuthorityInput): string | null {
  ...
  if (isSandboxOnlyMsg(msg)) {
    ...
    if ("PlaceHomeStation" in msg) {
      const placement = msg.PlaceHomeStation;
    ...
    if ("ExchangePrivate" in msg) {
      const exchange = msg.ExchangePrivate;
...
function roomMessageRefusal(input: TurnAuthorityInput, actor: string): string | null {
  ...
  if ("PlaceHomeStation" in msg) {
    ...
    const { company_id } = msg.PlaceHomeStation;
  ...
  if ("ExchangePrivate" in msg) {
    const { player, private_id } = msg.ExchangePrivate;
  ...
  if ("ProposePrivatePurchase" in msg) {
    return proposePrivatePurchaseRefusal(state, msg.ProposePrivatePurchase, actor);
  }
  ...
  if ("RescindPrivatePurchase" in msg) {
    return rescindPrivatePurchaseRefusal(state, msg.RescindPrivatePurchase, actor);
  }
  if ("RescindTrainPurchase" in msg) {
    return rescindTrainPurchaseRefusal(state, msg.RescindTrainPurchase, actor);
  }
  if ("RescindPrivateTrade" in msg) {
    return rescindPrivateTradeRefusal(state, msg.RescindPrivateTrade, actor);
  }
  ...
  if ("BuyKanawhaLicense" in msg) {
    const { protocol_id } = msg.BuyKanawhaLicense;
...
function consentAnswerRefusal(
  ...
  if ("AnswerPrivatePurchase" in msg) {
    ...
    return answerPrivatePurchaseRefusal(state, msg.AnswerPrivatePurchase, actor);
  }

  /* #1593: the trade's answer is the OTHER party's -- whichever of buyer and seller did not propose. */
  if ("AnswerPrivateTrade" in msg) {
    return answerPrivateTradeRefusal(state, msg.AnswerPrivateTrade, actor);
  }
  ...
  if ("AnswerTrainPurchase" in msg) {
    ...
    return answerTrainPurchaseRefusal(state, msg.AnswerTrainPurchase, actor, mapGrid);
  }
