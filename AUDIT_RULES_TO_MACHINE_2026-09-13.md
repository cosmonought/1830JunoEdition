...
| Level Playing Field | Owner-defined (7 seats, N&W/PMQ, JK, 20% double, 7-trains). Internally consistent with `gameVariants.ts`; the rulebook has no counterpart. Double-certificate half-sale rule (`doubleCertificate.ts`) encoded as ruled. | PASS (vs owner spec) |
