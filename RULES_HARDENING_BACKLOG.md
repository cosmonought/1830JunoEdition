...
**10** after the Unpredictable Revenue certification closure (UR-8, 2026-09-25; 9 was the Gentle Rust certification closure's, GR-5, 2026-09-24, #1705; 8 was the Stage-10 closure pass's, 2026-09-23, #1698; 7 was Stage 9's, #1680; 6 was Stage 8.5's). *(Historical note, kept as written:
...
implemented behaviour, not a final ruling on those points.)* *(2026-09-24, later: the buyer's train is now RULED —
OD-UR-5(a) = 5a-1, **D-48**: the ordinary train above is the rule — and the owner clarified it the same day: the train
keeps only its synthetic-origin provenance (#1673, below), for the +1 supply accounting, never a supernatural status, and
the Blood Price — an intercorporate purchase, not a Bank / Depot purchase — changes no phase. Whose share price moves is
still OPEN — OD-UR-5(b).)* *(Later the same day: RULED — OD-UR-5(b), **D-50**: the BUYER's marker moves Left 1 / Down 1
and the seller's does not, so "share-price consequences unchanged" above — #1090's seller move — is superseded;
UR-F22.)*
...
the run, never a client request (OD-UR-1, D-37); *(UR-3)* the taken train is permanently removed from the game — not the
depot, not the Bank Pool — and the phase never falls back across it (OD-UR-13, D-46). **CARCOSA AWAITS:** the gift never advances the phase (OD-UR-3, D-39);
...
decided. *(2026-09-24:)* the buyer's train is ORDINARY — the Blood Price cures the gilded / Carcosan status, so the second
sentence and "remains a ghost train and STILL disappears on the same D-triggered schedule" are superseded (OD-UR-5(a),
D-48 — clarified the same day: the "ghost" superseded is the supernatural status, which ends at the Blood Price; the
train keeps only its synthetic-origin provenance, for the +1 supply accounting, and the Blood Price, an intercorporate
purchase, changes no phase); a sale names the copy, and only the gilded copy's sale is the Blood Price (OD-UR-5(c), D-48); ~~**whose share price
moves is still OPEN** (OD-UR-5(b))~~ *(decided later the same day — OD-UR-5(b), D-50: the buyer's marker only; #1090's
seller move superseded, UR-F22)*.
...
Status, per variant (GR-5, 2026-09-24; UR-8, 2026-09-25): **Gentle Rust (standalone) — `CERTIFIED — CLOSED 2026-09-24 at
...
design: `VARIANT_CERT_UNPREDICTABLE_REVENUE_AUDIT_2026-09-24.md` (rev 7 — rev 3 records UR-3's implementation, rev 4 OD-UR-10's ruling, rev 5 UR-3's completion and the OD-UR-5 / OD-UR-6
rulings, rev 6 the owner's clarification of OD-UR-5(a), rev 7 OD-UR-5(b) and the OD-UR-6.1 per-train rule; *rev 8 UR-4's implementation, rev 9 UR-4's completion and UR-5's implementation, rev 10 UR-5's completion and UR-6's implementation, rev 11 UR-6's completion and UR-7's implementation*; UR-7's evidence: `VARIANT_CERT_UNPREDICTABLE_REVENUE_CERTIFICATION_2026-09-25.md`). Verdict **B —
specification complete (every owner decision made, rev 7) / implementation defects found** *(C — specification
incomplete, owner decisions required — through rev 6)*. 62 normative clauses (UR-N1 … UR-N62); matrix (clauses, rev 2): 30 with
evidence already at the bar, 12 correct but needing a live test, 16 defects, 4 owner decisions (rev 1: 31 / 12 / 10 / 9;
rev 4: 29 / 12 / 17 / 4 — UR-N6 moved to DEFECT UR-F20 by OD-UR-10; rev 5: 29 / 13 / 19 / 1 — UR-N50 to a live test,
UR-N54 and UR-N58 to DEFECT, by OD-UR-5 and OD-UR-6; rev 7: 29 / 13 / 20 / 0 — UR-N51 to DEFECT UR-F22 by OD-UR-5(b): no
owner-decision row remains; *rev 11, recomputed in UR-7 against the constructed game: clauses 59 certified / 3 correct but
needing a live test / 0 defects / 0 owner decisions / 0 deferred; interactions and invariants 25 / 2 / 0 / 0 / 1 — X14,
S10-21's completed game, deferred*)
...
(**D-45**). *(During UR-3, 2026-09-24:)* OD-UR-13 the Mark's taken train is permanently removed from the game — never the
depot or the Bank Pool — and phase progression is monotonic for every removal path (**D-46**). *(2026-09-24,
documentation only:)* OD-UR-10 an exact $5 tie of the modified revenue rounds toward the printed revenue — 10-C
(**D-47**); ~~not implemented (UR-F20 → UR-7)~~ *implemented in UR-7 (2026-09-25, uncommitted — UR-F20 `RESOLVED`)*. *(2026-09-24, documentation only:)* OD-UR-5(a) the Blood Price buyer's
train is ORDINARY — an additional ordinary train; clarified the same day: its synthetic-origin provenance survives only
for the +1 supply accounting, and the Blood Price, an intercorporate purchase, changes no phase — and OD-UR-5(c) the sale
names the copy (**D-48**); OD-UR-6 the revenue actually paid (6.1, in
principle), a Mark-nullified route not earned (6.2), a synthetic gift not a purchase and a Blood Price acquisition one
(6.3) (**D-49**). *(2026-09-24, documentation only:)* OD-UR-5(b) the Blood Price's market consequence falls on the
BUYER only — the buyer's marker moves Left 1 / Down 1, the seller's does not, never both (**D-50**; #1090's seller move
superseded — UR-F22); OD-UR-6.1's per-train question — per-train statistics use the printed value of the train's
successfully completed route, not an allocation of the paid total (**D-51**). **OPEN — later owner review:**
~~OD-UR-5(b) (whose share price moves) — the only decision left open, beside OD-UR-6.1's isolated per-train allocation
sub-question~~ **none — every Unpredictable Revenue owner decision is made (rev 7)**. *(As first recorded: OD-UR-5 (the Blood Price: the buyer's train,
whose share price moves, identical-model copies), OD-UR-6 (the statistics' basis), OD-UR-10 (rounding ties); OD-UR-10
decided since — D-47; OD-UR-5 (a), (c) — D-48 — and OD-UR-6 — D-49; OD-UR-5(b) — D-50 — and the OD-UR-6.1 per-train
question — D-51.)* **UR-3 is unblocked** — it has every
...
|---|---|---|---|
| UR-F1 | HIGH | the Yellow Sign never reaches the board in a room: its request is dispatched from inside the log drain and refused by #1407's catch-up guard (static trace — reproduce first) | **`RESOLVED` on pinned tables — UR-3 (`9d0cf3a`)**: the stage is the run's own entry; nothing dispatched; Firestore (unpinned) residual → S10-11 |
| UR-F2 | HIGH | the sign's request is not bound to its run (omission; delay past a phase change or Buy Trains; another corporation; any step; the window judged at application) — `messageSchema.ts` #1451's "recorded as deferred" was never tracked here | **`RESOLVED` on pinned tables — UR-3**: every client request refused (ingress, reducer gate, arm); Firestore residual → S10-11 |
| UR-F3 | HIGH | no variant gate: a table without the variant applies the Mark through hosted ingress | **`RESOLVED` on pinned tables — UR-3** |
| UR-F4 | HIGH | a Carcosa gift above the phase moves the derived phase without a phase change; the first real 6 / D then rusts nothing and a ghost D never gets a doom clock | **`RESOLVED` — carried in UR-3 by the owner's brief** (`derivePhase` skips ghosts; every table) |
| UR-F5 | MEDIUM | the Mark's route attribution after a Gentle Rust Final Run destruction | **`RESOLVED` — UR-3** (run path; `runWithoutTrain` aligned to the fleet as it ran) |
| UR-F6 | MEDIUM | the narration judges the Mark on the pre-run fleet | **`RESOLVED` — UR-3** (narration, fleet-loss notices and statistics read `last_run_yellow_sign`) |
| UR-F7 | MEDIUM | a run without `train_indices` leaves the Mark nothing to nullify | **`RESOLVED` on pinned Unpredictable Revenue tables — UR-3** (the authority's pairing) |
| UR-F8 | MEDIUM | statistics: printed basis, nullified runs booked as earned, synthetic gifts counted as purchases / the first Diesel | ~~UR-5 — OD-UR-6 **decided** (D-49: the revenue actually paid, in principle; a nullified route not earned; a gift not a purchase, a Blood Price acquisition a purchase) — not implemented; OD-UR-6.1's per-train allocation is an isolated open sub-question~~ *(decided — D-51: per train, the printed value of the completed route; a nullified route contributes nothing)* **`RESOLVED` — UR-5 (2026-09-25; owner-gated, committed and pushed as `0d8157e`)**: runs booked as they settled — the corporation / turn statistics (lifetime revenue, Workhorse, Juggernaut, Revenue-per-OR chart, White Elephant, Little Engine) on `last_route_revenue`, the train / route statistics (Master of the Line, the ledger's earned / runs) on each completed route's printed figure; a Mark-nullified route nothing (the pinned run's own entry; the unpinned legacy request amends the turn's accepted booking and never rewrites what the Dividends step paid); the Carcosa gift — the copy that arrives gilded — in no purchase statistic (Fleet Admiral, the ledger's paid, The Early Adopter), the ledger still holding it at $0 (#1431); the Blood Price the buyer's purchase, a Diesel's Early Adopter included; a cured copy bought from the Bank Pool an ordinary purchase (`unpredictableRevenueStats`, `yellowSignRunBoundStats`); derived history only, no version effect |
| UR-F9 | LOW | The Cowboy reads the natural flavour line, not the printed one | **`RESOLVED` — UR-5 (2026-09-25; owner-gated, committed and pushed as `0d8157e`)**: the printed line — the pinned run's applied stage (`narrateRunYellowSign`), the legacy resolution unpinned, no line for a $0 run (`unpredictableRevenueStats` UR-F9) |
| UR-F10 | **CLOSED — intended** | the Mark's award is minted (`MINTS_BY_DESIGN`) — OD-UR-4 / D-40: variant law, not a defect; only a stale test comment remains | UR-5 (the comment) — **corrected in UR-5** (`moneyConservation.test.ts`, comments only) |
| UR-F11 | **CLOSED — ruled** (2026-09-24) | S9-3's verbatim rule contradicts the implementation (the expiry — resolved by OD-UR-2, the gap is now UR-F18; the Blood Price — OD-UR-5 OPEN; *rev 5: the buyer's train — OD-UR-5(a), D-48 — decided for the implementation; the market consequence — OD-UR-5(b) — still OPEN*; *decided — D-50, the buyer only → UR-F22*) | — (every contradiction ruled: UR-F18 fixed, UR-F22 open) |
| UR-F12 | LOW | the gilded chip's tooltip is stale ("until this Operating Round ends") | ~~UR-6 (text per OD-UR-2's lifetime)~~ **`RESOLVED` — UR-6 (2026-09-25; owner-gated, committed locally as `cd889cb`, not pushed)**: `CARCOSA_CHIP_TOOLTIP` — the exemption while gold-trimmed, no Diesel trade-in, the fog at the end of the Operating Round set after the doom trigger, the buyer's Blood Price; `alt` "Gold-trimmed Carcosa train" (`unpredictableRevenueDisclosure`, `batch59`) |
| UR-F13 | LOW | the Rules Reference carries one sentence on the variant | UR-6 — OD-UR-8 decided (8-B); the Blood Price text after OD-UR-5 *(fully decided — D-48, D-50)* **`RESOLVED` — UR-6 (2026-09-25; owner-gated, committed locally as `cd889cb`, not pushed)**: two tagged blocks — Dividends (one die per turn on the total, the face table, "rounded to the nearest $10", printed values for route choice, the paid figure; the Yellow Sign's CONSEQUENCES only — the taken train removed from the game for good, half its face value) and Buy Trains (the gold-trimmed train, the fog, the Blood Price paid by the buyer, the cured ordinary train, no phase change, the copy named); the trigger conditions and odds stay hidden (D-42); no tie direction — the tie sentence waits for UR-7 (UR-F20) *(added in UR-7: an exact halfway result is rounded toward the printed route total)* (`unpredictableRevenueRulesReference`) |
| UR-F14 | LOW | "Unpredictable revenue" vs "Unpredictable Routes" | UR-6 — OD-UR-12 decided ("Unpredictable Revenue") **`RESOLVED` — UR-6 (2026-09-25; owner-gated, committed locally as `cd889cb`, not pushed)**: the host setup and the room list say "Unpredictable Revenue"; the sentence-case label lists keep their case ("Unpredictable revenue" beside "Gentle rust (easier)") |
| UR-F15 | INFO | a pinned seedless run would use the legacy hash (unreachable through the server) | UR-3 (optional) — **not taken; stays `OPEN` (INFO)** |
...
| UR-F17 | MEDIUM | a gilded 5 / 6 can be traded in for a Diesel, leaving the gilding, provenance, curse and doom clock behind and a synthetic train in the Bank Pool | **`RESOLVED` — carried in UR-3 by the owner's brief** (multiset: an ordinary copy stays tradable; $800 and LPF $750; atomic refusal) |
| UR-F18 | MEDIUM | *(rev 2)* the fog is collected as a run stage (the Carcosan corporation's first run after set N+1, through the sign request) where OD-UR-2 rules an automatic removal at the END of set N+1, at the OR-set boundary — do **not** restore `N+1 + on-run` | **`RESOLVED` — UR-3** (`fogAtSetEnd` in the round machine; every exit of the set; pinned and unpinned) |
| **UR-F19** | MEDIUM | *(new, UR-3)* the Mark removes the taken train and puts it nowhere: the derived depot takes it back as phantom stock when it is of the phase's tier, and when it was the only train of that tier in play the phase falls back a tier (probe: phase 3 → 2, the depot offers 2-trains again) — pre-existing since #1046, reachable now that the Mark lands | **`RESOLVED` on pinned Unpredictable Revenue tables — UR-3, under OD-UR-13 (D-46)**: the run-bound Mark records the train on `removed_trains` (never the Bank Pool); `derivePhase` counts it toward the phase and `depotInventory` off the depot, so the phase never falls back and the copy never returns to sale; replay re-derives it; statistics still `taken` (`yellowSignMarkRemoval`, `yellowSignRunBoundStats`). Other removal paths checked — no broader regression (audit, "UR-3 implementation (rev 3)"). Unpinned residual → S10-11 |
| **UR-F20** | MEDIUM | *(new, OD-UR-10 = 10-C)* an exact $5 tie of the modified revenue rounds half up (`roundToTen`) where OD-UR-10 rules it toward the printed revenue — only the +10% face differs, at printed ≡ $50 (mod $100): $50 pays $60 (rule: $50), $150 pays $170 ($160), $250 pays $280 ($270) | ~~`OPEN` — UR-7 (the rounding table and the tie rule; not implemented)~~ **`RESOLVED` — UR-7 (2026-09-25, uncommitted, awaiting the owner's gate)**: `rollTurnRevenue` judges the tie on the exact product (`printed × percent ≡ 500 mod 1000`) and sends it to `roundRevenueTowardPrinted` (toward printed); every other amount unchanged; integers only; the Rules Reference's tie sentence (`revenueTieRounding`, the certification game §1, `unpredictableRevenueRulesReference`); every table; the corpus unchanged (0 exact ties among 247 Unpredictable Revenue runs); replay-semantic, owed to the 9 → 10 boundary (UR-8) |
| **UR-F21** | MEDIUM | *(new, OD-UR-5(c) = 5c-2)* a seller holding a gilded and an ordinary copy of one model cannot sell the ordinary copy as an ordinary sale: the Carcosan transfer is model-level (`isCarcosanTransfer`), so any sale of that model charges the Blood Price, absolves the seller and burns the gilding — where the ruling says the sale names the copy (the ordinary copy's sale: no Blood Price, the gilded copy stays gilded; the gilded copy's sale: the Blood Price); The Redeemer reads the same model-level predicate | **`RESOLVED` — UR-4 (2026-09-25, owner-gated, committed and pushed as `a6183b5`)**: the optional `gilded` on `BuyTrainFromCorporation` / `ProposeTrainPurchase` / the offer / its derived settlement names the copy; an unnamed sale of a model held both gilded and ordinary is refused atomically at every moment (proposal, answer, settlement, chart step, hosted ingress); the ordinary copy's sale leaves the gilding, curse, deadline and the gilded copy's provenance; The Redeemer reads the gilding the sale burned (`carcosaBloodPrice`, `carcosaBloodPriceStats`, `carcosaBloodPricePanel`); every table; replay-semantic, owed to the 9 → 10 boundary (UR-8) |
| **UR-F22** | MEDIUM | *(new, OD-UR-5(b) = the buyer only)* the Blood Price moves the SELLER's stock marker Left 1 / Down 1 — #1090's market arm (`sandboxSession.ts:2674–2695`: "THE SELLER MOVES, NOT THE BUYER") — where the ruling moves the BUYER's marker and gives the seller no separate movement (never both); the Train Purchase panel's warning ("The selling corporation's share price will immediately drop", `TrainPurchasePanel.tsx:1388–1389`) and the narration's design note ("[Selling Corp]'s stock dropped", `App.tsx:6904`) repeat the superseded rule, and `bloodPriceArrival` / `stage103CompositionCoupling` pin it — **do not restore the seller move from any of them** | **`RESOLVED` — UR-4 (2026-09-25, owner-gated, committed and pushed as `a6183b5`)**: the chart arm moves `buyer_protocol_id` Left 1 / Down 1 and never reads or writes the seller's token; the Train Purchase warning names the buying corporation and shows only for the gilded copy; the consent prompt names the copy and the buyer's drop; the market arm's, `projectBloodPriceMove`'s and the narration's design notes marked superseded; `bloodPriceArrival`, `stage103CompositionCoupling`, `stage103bChartCoreAtomicity`, GR-2 S8 and `batch60` now pin the BUYER (`carcosaBloodPrice` B, I2, I4); every table; replay-semantic, owed to the 9 → 10 boundary (UR-8). The rest of the Blood Price copy (the Rules Reference, UR-F13) stays UR-6's |

Roadmap: **UR-2** owner spec review (**part 1 done** 2026-09-24; part 2 = OD-UR-5, -6, -10 — OD-UR-13, raised by UR-3, was
decided during it, D-46; OD-UR-10 decided 2026-09-24, D-47; OD-UR-5 (a), (c) and OD-UR-6 decided 2026-09-24, D-48 and
D-49; OD-UR-5(b) and the OD-UR-6.1 per-train question decided 2026-09-24, D-50 and D-51 — **UR-2 complete: every owner
decision made**) → **UR-3** Yellow Sign authority, including the fog's boundary removal (**COMPLETE 2026-09-24 —
owner-gated, committed and pushed as `9d0cf3a`** — with UR-4's UR-F4 and UR-F17 by the owner's brief, and UR-F19 under
OD-UR-13) → **UR-4** Carcosa lifecycle, now only the Blood Price (OD-UR-5(a) pinned live — the +1 supply accounting kept and the
cured train otherwise ordinary, no owner question left on it; (c) — UR-F21; (b) — UR-F22, the buyer's marker; no owner-decision blocker)
(**COMPLETE 2026-09-25 — owner-gated, committed and pushed as `a6183b5`**; the UR-4 block below)
→ **UR-5** money and statistics (OD-UR-6 fully decided — UR-F8: paid turn totals, printed completed routes per train;
no owner-decision blocker) (**COMPLETE 2026-09-25 — owner-gated, committed and pushed as `0d8157e`**; the UR-5 block below) → **UR-6** UI / copy / Rules Reference (**COMPLETE 2026-09-25 — owner-gated, committed locally as `cd889cb`, not pushed**; the UR-6 block below) → **UR-7** certification
evidence and the tie rule (the constructed legal game — no owner-decision blocker since D-50 / D-51 — S10-21; the rounding table and OD-UR-10's 10-C tie rule — UR-F20,
D-47) (**IMPLEMENTED 2026-09-25 — uncommitted, awaiting the owner's gate**: UR-F20 `RESOLVED`, G1 + G0 built, the matrix recomputed; S10-21's completed game **not built** — routed to UR-8 or a named follow-up; the UR-7 block below) *(UR-8: **COMPLETE — owner-gated, committed and pushed as `ff7a04b`**)* → **UR-8** the one 9 → 10 boundary and
closure *(**DONE 2026-09-25 — uncommitted, awaiting the owner's full-suite gate**: `RULES_ENGINE_VERSION` 10, row 10, S10-21 closed as superseded — "Variant Certification 1B — CLOSED" below)*. **Unpredictable Revenue, and Gentle Rust + Unpredictable Revenue, remain
NOT certified.** *(UR-8: superseded — both CERTIFIED at `RULES_ENGINE_VERSION` 10.)* Evidence-quality note for UR-7 (audit Appendix C): `yellowSignIngress.test.ts` case 2 is evidence for
the ingress normalizer's placement only — its bare request is `applied` because the constructed board is unsettled, not
because the sign acted. *(UR-3: resolved — case 2 now proves the rewrite on a run with a real route, whose seed the
normalizer replaces, and a pinned table refuses the request before normalizing.)* Recorded outside the variant (audit Appendix C, not investigated): `server/src/gameServer.ts:268`
...
**UR-3 — the Yellow Sign's authority (2026-09-24 — COMPLETE: owner-gated — the owner's full repository Jest gate passed —
committed and pushed as `9d0cf3a`).** On a pinned
Unpredictable Revenue table the stage is the last step of the accepted run's own transition (`settleRunYellowSign`,
after the Run → Dividends settlement, before bankruptcy), read from the run's committed seed and recorded on the
corporation as `last_run_yellow_sign`; the client `YellowSignEvent` is refused there (ingress, reducer gate, arm, shell
refusal line) and the shell dispatches none; the Mark reads the post-settlement fleet and nullifies the taken train's own
route; the run's breakdown is the authority's pairing; the fog is `fogAtSetEnd`, an OR-set-boundary transition at the end
of set N+1; `derivePhase` ignores synthetic trains (UR-F4); `exchangeableTrains` excludes gilded copies by multiset
(UR-F17); and — OD-UR-13 decided during the slice (D-46) — the Mark's taken train is recorded as removed from the game
(`removed_trains`, never the Bank Pool), which `derivePhase` counts toward the phase and `depotInventory` off the depot, so
the phase never falls back across the Mark and the copy never returns to sale (UR-F19). Unpinned boards keep the legacy
request path byte for byte (the corpus's stored entries). `RULES_ENGINE_VERSION` stays **9**; the replay semantics are owed
to the 9 → 10 boundary (UR-8, Part E). Evidence: seven new suites (`yellowSignRunBound`, `yellowSignRunBoundStats`,
`carcosaFogBoundary`, `carcosaSyntheticPhase`, `carcosaGildedExchange`, `unpredictableRevenueStandardControls`,
`yellowSignMarkRemoval` — 92 tests; 67 of them fail on the untouched baseline) and seven updated where their contract
changed (`yellowSignAuthority`, `yellowSignIngress`, `stage95Rulings`, `stage95GhostLimit`, `batch48`, `batch60`, `batch61`,
each with a UR-3 note). OD-UR-13's monotonic-phase invariant was checked against every other train-removal path (rust,
Gentle Rust destruction, discard, trade-in, sale / Blood Price, fog / doom, bankruptcy) and measured on the 18-file corpus
(0 phase decreases): no broader regression found. Deferred and recorded: the Firestore residual (S10-11 — now including
the legacy Mark's missing removal record), a greyed gilded chip in the Diesel-exchange row, the fog's formal notice and a
player-facing line that the Mark's train is gone for good (UR-6), and the forced-withhold derivation for a trainless
corporation whose run still earned (S10-27). **Unpredictable Revenue, and
Gentle Rust + Unpredictable Revenue, remain NOT certified.** Audit: rev 3.

**UR-4 — the Blood Price, copy by copy, paid by the buyer (2026-09-25 — COMPLETE: owner-gated — the owner's full
repository Jest gate passed — committed and pushed as `a6183b5`).** OD-UR-5 (D-48, D-50) on every table, pinned and unpinned (the corpus holds no gilded train,
no provenance marker and no Carcosan transfer — measured). **(c) UR-F21:** the sale names the COPY — one optional boolean,
`gilded`, on `BuyTrainFromCorporation` and `ProposeTrainPurchase` (schema `"bool?"`), carried by the offer and its derived
settlement; multiset copy selection (`resolveSaleCopy` in `trainSaleAuthority.ts`), no persistent train identity; absent
is legal only where it cannot be ambiguous; an unnamed sale of a model the seller holds both gold-trimmed and ordinary is
refused atomically at the proposal, the answer, the settlement, the chart step and hosted ingress; consent includes the
copy. Only the gilded copy's sale burns the gilding, clears the curse and the clock, and moves the chart; provenance
leaves with the copy that has it (never the gilded copy's with an ordinary copy). The Redeemer reads the gilding the sale
burned — the one statistics coupling to the model-level predicate; nothing else in the statistics moved. **(b) UR-F22:**
the Blood Price moves the BUYER's marker Left 1 / Down 1 and never the seller's; the warning, the consent prompt and the
design notes updated, and the five suites that pinned the seller now pin the buyer. **(a):** pinned live — the cured train
is ordinary (limit, running, rust, second sale, Diesel trade-in, fog, curse, clock) and keeps only its synthetic-origin
provenance (`ghost_trains`, supply accounting only — no field split; its phase / real-D reads are the Depot-purchase
reason, not a status); the Blood Price turns no phase, rust, Gentle Rust mark, Depot shelf, 18XX+ era or real-D trigger (a
6, a D, an LPF 7; Gentle Rust, 18XX+, LPF), and the first REAL train of the tier still does all of it. **Found and fixed
narrowly (intrinsic to 5a-1):** a cured copy traded in for a Diesel (or, on a constructed board, discarded) entered the Bank
Pool as a PRINTED train — the Depot lost a printed copy it never sold — and left its marker behind to swallow the
corporation's next copy of the model (#1675's hazard); the pool now carries the additional copy's provenance
(`returned_ghost_trains`, optional, absent on every board without one), read by the Depot tally, the phase and the real-D
check (`printedPooledTrainsByTier`), and a pool purchase hands it to the buyer. Ordinary rust never meets a synthetic copy
(the gift is always a 5, 6, 7 or D); the discard path is unreachable for a cured copy (limits from phase 5 on are 2 and
never fall) and is fixed and pinned on a constructed board. **The normal UI names the copy** — the Buy Trains roster
marks the gold-trimmed badge and each badge sends its own copy (only when the seller holds a gilded copy of the model);
UI parity Part C U-42. `RULES_ENGINE_VERSION` stays **9**; the semantics are owed to the 9 → 10 boundary (UR-8, Part E).
Evidence: three new suites (`carcosaBloodPrice` 42, `carcosaBloodPriceStats` 2, `carcosaBloodPricePanel` 6 — 50 tests;
30 fail / 20 pass on the untouched baseline) and six updated where their contract changed (`bloodPriceArrival`,
`stage103CompositionCoupling`, `stage103bChartCoreAtomicity`, `gentleRustTransactionLocks` S8, `stage95GhostLimit` E,
`batch60`), each with a UR-4 note; focused Jest 263 suites / 5,228 tests green; tsc (frontend, server) 0; build 49 warning
lines, identical to the baseline; the 18-file corpus 0 differences in 3,763 engine applications (Y8V `b4fae877c35604fe`,
3XD `74db6e4bad736fec`). **Deferred and recorded:** (UR-5) Carcosan Railways' blurb — "never paid the Blood Price to be rid
of it" — predates OD-UR-5(b): a seller released by a buyer's Blood Price never pays it, so whether its president keeps the
accolade is a statistics question for UR-5 (the history still excludes only the paying, buying, president, as before)
*(answered in UR-5: it keeps it — the logic is #1421's and correct; only the blurb is stale → UR-6)*;
(UR-6) the Activity Log's offer / trade lines name the model, not the copy, and the Rules Reference's Blood Price text
(UR-F13). **Unpredictable Revenue, and Gentle Rust + Unpredictable Revenue, remain NOT certified.** Audit: rev 8.

**UR-5 — the statistics of an Unpredictable Revenue game, as ruled (2026-09-25 — COMPLETE: owner-gated — the owner's full
repository Jest gate passed — committed and pushed as `0d8157e`).** OD-UR-6 (D-49, D-51) in `utils/gameHistory.ts` — derived history only: no reducer,
board, message, digest or version change. **UR-F8:** a run is booked as it SETTLED (`RunBooking`), at two levels — the
corporation / turn statistics (lifetime revenue and The Workhorse, The Juggernaut, the Revenue-per-OR chart, and through
lifetime revenue White Elephant and Little Engine) on `last_route_revenue`, the revenue actually paid (what the Dividends
step declares, audit C1); the train / route statistics (Master of the Line, the fleet ledger's earned and runs) on each
completed route's PRINTED figure off `last_run_breakdown` — never an allocation, and not expected to add up to the paid
total. A Mark-nullified route contributes nothing: on a pinned table the Mark lands in the run's own entry, which has
already taken the route out, so it is never booked; on an unpinned table the later legacy request amends the turn's
ACCEPTED booking before any tally is drawn (the taken train's route leaves the completed routes — the reducer's own
`runWithoutTrain` remainder; the paid figure follows the board unless the Dividends step already paid the run out). The
tallies are drawn from the bookings once, after the replay, with the old strict "must beat" rule. **Acquired is not
bought:** the Carcosa gift — the one copy that arrives gilded (`applyYellowSignOutcome` writes the train, its provenance
and its gilding together; no purchase delivers a gilded copy) — joins no purchase statistic (Fleet Admiral, the ledger's
paid, The Early Adopter), while the fleet ledger still holds it at $0 (#1431's n is "the number of trains the corporation
held"); every other roster gain is a purchase, so the Blood Price is the BUYER's genuine purchase (a Diesel's Early
Adopter included; the seller's copy is `sold` and the seller credited with none) and a cured copy bought from the Bank
Pool, provenance and all, is an ordinary purchase. **UR-F9:** The Cowboy counts the printed line — `narrateRunYellowSign`
on a pinned table, the shell's legacy `resolveFlavourLine` call unpinned, no line for a $0 run. **UR-F10's stale comment**
corrected (`moneyConservation.test.ts`, comments only; the case's title kept — Batch 7.3 / 7.5 cite it). **Carcosan
Railways under OD-UR-5(b):** the logic is correct (#1421 — the buyer pays the Blood Price and is The Redeemer; a released
seller never paid, so its president keeps the accolade — pinned); only the blurb's "to be rid of it" is stale → UR-6
(audit Appendix B item 13). Every other statistic needed nothing (the dividend family already reads the money paid; Salt /
Sugar, train spend, Phase Rusher and the rust cause are unchanged). Evidence: `unpredictableRevenueStats.test.ts` (new, 46
tests — 22 fail / 24 pass on `a6183b5`: A / B paid vs printed, C / C2 / C3 the Mark on both roads, G / J the gift of a 6
and of a D, H / K the Blood Price of a 6 and of a D (hosted offer flow, replayed), I the Bank Pool, UR-F9, Carcosan
Railways, L standard controls, M replay / restore / undo with no random draw, and a real replay of JUNO-Z6C-494);
`yellowSignRunBoundStats.test.ts` (two UR-3 pins moved to the ruled basis, with a UR-5 note); focused Jest 157 suites /
3,199 tests green; tsc (frontend, server) 0; build 49 warning lines, identical to the baseline; the 18-file corpus 0
differences in 3,763 engine applications (Y8V `b4fae877c35604fe`, 3XD `74db6e4bad736fec`), and its DERIVED STATISTICS
identical on every standard file, moved only on the four Unpredictable Revenue logs (CW7, FCJ, Z6C, Z6C-494) in the ruled
directions (paid corporation revenue; two phantom Cowboy run-ins on refused $0 runs removed). `RULES_ENGINE_VERSION` stays
**9**; no Part E row (the statistics basis is named NOT a rule in the owed row 10). **Deferred and recorded:** Part C
**U-43** (pre-existing, non-UR statistics residuals); (UR-6) Carcosan Railways' blurb and, under the variant, the
Revenue-per-OR chart's hint and the ledger's "Runs" column (audit Appendix B items 13, 14) *(both done in UR-6)*. **Unpredictable Revenue, and
Gentle Rust + Unpredictable Revenue, remain NOT certified.** Audit: rev 9.

**UR-6 — the players are told what the authority does: UI / copy / Rules Reference parity (2026-09-25 — COMPLETE:
owner-gated, committed locally as `cd889cb` "Polish Unpredictable Revenue player-facing rules", not pushed — local `main`
one commit ahead of `origin/main`, as the owner confirmed; at first uncommitted, awaiting the owner's full repository Jest
gate).** Replay-neutral presentation: no reducer rule, server
authority, message, schema, state field, digest, statistic, log, fixture or golden changed; the engine files touched
(`sandboxSession.ts`, `gameState.ts`, `gameVariants.ts`, `yellowSign.ts`, `trainDiscard.ts`, `utils/gameHistory.ts`) differ
only in comments — verified by transpiling both trees with comments removed — so no corpus reconciliation was needed.
**D1 (the independent UR-4 review) + U-42's copy:** `utils/saleCopyDisclosure.ts` (new) asks the authority's own
predicates (`isCarcosanTransfer`, `resolveSaleCopy` / `saleCopies`); the consent prompt and the pending-offer view (one
`TrainTradePrompt`) disclose the Blood Price from the shell's derived `bloodPrice` — an unnamed offer for the seller's only
gold-trimmed copy included — and never for an ordinary copy; the Activity Log's offer, answer and trade lines name the
copy (the gold-trimmed copy: "buying it is the Blood Price" … "paying the Blood Price: it is an ordinary 6-train now, and
B&O is released from the Carcosan curse"; an ordinary copy beside it: "not the gold-trimmed one, so no Blood Price"; every
other sale byte for byte as before); the buyer-only market line (#1090, UR-4) is unchanged and names the buyer.
**UR-F13:** the Rules Reference's one sentence is replaced by two blocks tagged "Unpredictable revenue" — Dividends (the die
table, one roll per turn on the total, "rounded to the nearest $10", printed values for route choice, the paid figure
paid or withheld, private income untouched, an undo does not re-roll; the Yellow Sign's consequences only — the taken
train's route earns nothing, the train is removed from the game for good, half its face value; Gentle Rust's Final Run
never taken, where both are on) and Buy Trains (the gold-trimmed train — extra, no limit slot, never the phase, no Diesel
trade-in, the curse; the fog at the end of the set after the doom trigger; the Blood Price — the buyer pays and moves 1
left / 1 down, the seller does not move and is released, the buyer's train ordinary, no phase change, the copy named);
the Yellow Sign's trigger conditions and odds stay hidden (D-42 — pinned across every page); no tie direction.
**UR-F12:** `CARCOSA_CHIP_TOOLTIP`. **UR-F14:** "Unpredictable Revenue" on the host setup and the room list. **Awards:**
Carcosan Railways' blurb — "Saw the Yellow Sign touch their railroad, and never paid a Blood Price to buy a gold-trimmed
train." (logic unchanged); The Redeemer's already named the buyer (unchanged). **UR-3's deferrals:** the Diesel trade-in
row greys a gold-trimmed copy with its reason (`gildedExchangeCopies` / `gildedExchangeCopyReason`, presentation beside
GR-3's Final Run pair; `DieselExchangeOffer.gilded` absent on every board without one); the fog's ruled sound and film
play at the set boundary, once, guarded by `replayingHistory` (no modal was ruled or added); the Mark's train "removed
from the game for good" is said in the Rules Reference. **Appendix B:** items 1 – 10 annotated in place (comment-only), 12 –
14 changed; the independent review's D5 comment corrected. Evidence: two new suites
(`components/unpredictableRevenueDisclosure.test.tsx` 25, `components/unpredictableRevenueRulesReference.test.tsx` 15 —
25 fail / 15 pass on `0d8157e`);
four updated (`carcosaBloodPricePanel` — its proposal carries the shell's derivation; `batch59` — the chip's pins;
`batch60` — the fog cue's pins; two test comments), each with a UR-6 note; focused Jest 335 suites / 6,290 tests on a
clean clone of `0d8157e` plus the tree — 328 pass, and the 7 that read the gitignored local corpus (failing identically on
the baseline there) pass on the owner's checkout (125 tests); tsc (frontend, server) 0; build exit 0, 49 ESLint warning
lines identical to the baseline; `git diff --check` clean; an independent read-only review found no blocker (its two
should-fix wording items applied). `RULES_ENGINE_VERSION` stays **9**; no Part E row (UI / copy / Rules Reference are named
NOT rules in the owed row 10). **Deferred and recorded:** (UR-7) the Rules Reference's exact-$5 tie sentence, when 10-C
lands (UR-F20); an observation — the host's Yellow Sign debug chip names the stage windows and renders for any sandbox
host, including a pinned table where its force is ignored (debug tooling; UR-7 to re-assert UR-N62 on a hosted table, or
S10-11 retires it); two unreachable nits (audit "UR-6 implementation (rev 10)"). *(Both deferrals done in UR-7 — the tie
sentence, and the chip shown only where its force can act; the UR-7 block below.)* **Unpredictable Revenue, and Gentle Rust +
Unpredictable Revenue, remain NOT certified** — UR-7 and UR-8 remain. Audit: rev 10.

**UR-7 — 10-C, the debug chip, and the constructed certification game (2026-09-25 — IMPLEMENTED; COMPLETE: owner-gated
and committed and pushed as `ff7a04b` *(UR-8; written while uncommitted, awaiting the owner's full repository Jest
gate)*).** Start: local HEAD `cd889cb` = the owner-gated UR-6 baseline; local `main` ahead
of `origin/main` (`0d8157e`) by 1 expected commit (left alone, as the owner asked); the tracked tree clean; no
`.git/index.lock`. **UR-F20 `RESOLVED` — OD-UR-10 = 10-C (D-47):** `rollTurnRevenue` (`gameEngine/gameVariants.ts`), the
one rounding path every reader calls on the committed draw, judges an exact tie on the product (`printed × percent ≡ 500
mod 1000`) and sends it to the new `roundRevenueTowardPrinted(modified, printed)` — the ten nearer the printed total;
every other amount rounds exactly as before; integers only. On every printed multiple of $10 only the +10% face at
printed ≡ $50 (mod $100) pays differently — $10 less than half up ($55 → $50, $165 → $160, $275 → $270); the −10% tie
already went to printed; the six faces' mean now equals printed; faces 1 and 6 (the Yellow Sign's) never tie. Private
income stays outside the die; per-train statistics stay printed, corporation statistics paid; undo, replay and restore
reuse the committed draw. The Rules Reference's Dividends block adds "If the result lands exactly halfway between two $10
values, it is rounded toward the printed route total." — the hidden trigger and odds untouched (D-42). **UR-N62 — the
host's Yellow Sign debug chip:** debug / playtest tooling (#1128) that could never change canonical state on a pinned
table (the waiver is armed only on a local board; hosted ingress drops `debug_force` and refuses the request; the
reducer refuses it — re-pinned on a hosted table), but it RENDERED on pinned tables and its tooltips named the Sign's
phase windows and whom it visits next to a host who is also a player; `forcedSignToolInForce` (`yellowSign.ts`) now
gates the chip, its cycle and Ctrl+Shift+Y (`App.tsx`) to an unpinned Unpredictable Revenue board — S10-11's residual,
where #1128's force still acts. No new debug architecture; no bypass existed, so no blocker. **The constructed
certification game** (`utils/unpredictableRevenueCertificationGame.ts`, test support; `…CertificationGame.test.ts`, 56
tests) — a constructed, never-exported, never-blessed game in GR-4's manner, submitted through `RoomSession.submit` with a
scripted `mintSeed` (each draw armed for the run that needs it; an unarmed draw throws): G1 (Unpredictable Revenue +
Gentle Rust) — the exact tie at OR 3.1 with a refused shortfall run and an undo on the same draw; the Mark on C&O's Gentle
Rust grace turn (the Final Run 2 retired first, the Mark takes the 4 — OD-GR-3; removed for good; $150 minted; three
client requests refused; the trainless obligation); the Carcosa gift ABOVE the phase (a 6 in phase 5, nothing advances);
the first real 6 and D; tail A — the fog at the end of set 6, at the boundary; tail B — the Blood Price (the copy named,
the buyer pays and moves one cell left and one down, the seller released) and the cured train's ordinary life (limit,
run, Diesel trade-in to the pool with its provenance, a pool purchase, an ordinary sale, no fog); purchase statistics and
awards exact (paid corporation revenue, printed routes, the gift no purchase, the Blood Price the buyer's, Carcosan
Railways, The Redeemer, the Gravedigger and the Rust Belt); G0 — the standard control. Also new:
`utils/revenueTieRounding.test.ts` (23 — the owner's matrix A – H, the whole table and integer domain against an
independent oracle, the authority, the hosted path) and `utils/yellowSignDebugChip.test.ts` (14); updated:
`revenueRounding` (one oracle), `unpredictableRevenueRulesReference` (the tie pin). New suites: 93 tests — 33 fail / 60
pass on the UR-6 tree. **Matrix recomputed** (certification document §H – §J): clauses 59 / 3 / 0 / 0 / 0, interactions
and invariants 25 / 2 / 0 / 0 / 1 — no defect, no owner decision; the three live-test rows need harnesses, not rules.
**Corpus** (18 files): 0 differences in 3,763 engine applications; Y8V `b4fae877c35604fe`, 3XD `74db6e4bad736fec`; 0 exact
ties among 247 Unpredictable Revenue runs; nothing repinned. **Validation:** focused Jest 290 suites / 5,742 tests (cloud
clone, corpus present) and 58 suites / 1,260 tests on the owner's checkout — all pass; tsc (frontend, server) 0; build
exit 0, 49 ESLint warning lines identical to the baseline; `git diff --check` clean; an independent read-only review
found no blocker (its should-fix items and nits applied). `RULES_ENGINE_VERSION` stays **9** — 10-C is replay-semantic,
implemented at v9 on every table and owed to row 10 (item (7)); **do not deploy UR-7 to a server holding live v9 rooms
before UR-8**. No owner decision new or re-opened; OD-UR-9 stays deferred (no fresh random source in replay). **Deferred
and recorded:** **S10-21** — the completed Yellow Sign game NOT built (routed to UR-8 or a named follow-up — the
planner's call; see S10-21); **S10-27** reproduced live by the Mark (not fixed); **S10-11** — the server's `forced-sign`
room op is accepted from any member (inert on pinned tables; not changed). Evidence:
`VARIANT_CERT_UNPREDICTABLE_REVENUE_CERTIFICATION_2026-09-25.md`. **Unpredictable Revenue, and Gentle Rust + Unpredictable
Revenue, remain NOT certified** — UR-8 remains. Audit: rev 11.
*(UR-8: UR-7 passed the owner's full repository Jest gate and is committed and pushed as `ff7a04b`.)*

**Variant Certification 1B — CLOSED (UR-8, 2026-09-25): UNPREDICTABLE REVENUE IS CERTIFIED at `RULES_ENGINE_VERSION`
10 — standalone and with Gentle Rust (OD-GR-3).** Slices, each owner-gated: **UR-1** audit / design · **UR-2** owner
rulings (D-36 … D-51; every decision made) · **UR-3** the Yellow Sign's authority, the fog, the synthetic phase, the gilded
trade-in, the Mark's removal (`9d0cf3a`) · **UR-4** the Blood Price (`a6183b5`) · **UR-5** statistics (`0d8157e`) · **UR-6**
UI / copy / Rules Reference (`cd889cb`) · **UR-7** 10-C, UR-N62, the constructed certification game (`ff7a04b`) · **UR-8**
the one deliberate `RULES_ENGINE_VERSION` **9 → 10** boundary and this closure (uncommitted, awaiting the owner's
full-suite gate and commit). **Changelog row 10** names exactly eight replay semantics — (1) the run-bound Sign, client
requests refused on pinned tables (OD-UR-1, D-37); (2) the Mark on the post-settlement fleet, never a Final Run train
(OD-GR-3, D-36); (3) the fog at the end of OR set N+1 (OD-UR-2, D-38); (4) synthetic trains never the phase (OD-UR-3,
D-39); (5) no gilded Diesel trade-in (OD-UR-7, D-41); (6) the Blood Price names the copy, moves the buyer only, and cures
into an ordinary additional train with provenance-only supply tracking (OD-UR-5, D-48 / D-50); (7) 10-C (OD-UR-10, D-47);
(8) the Mark's train removed from the game, the phase monotonic (OD-UR-13, D-46) — and names the minted award (D-40), the
statistics basis (D-49 / D-51), the UI / copy / Rules Reference and naming (D-42, D-45), the undo rule (D-44), the seed
source (D-43) and the certification evidence as NOT rules. A v9-pinned room is held `incompatible` under every policy,
never reinterpreted; new rooms deal at 10 (`unpredictableRevenueClosure.test.ts`, which owns the current literal;
`gentleRustClosure` and four UR suites narrowed, version literals only). Across the boundary the 18-file corpus is
unchanged (0 differences in 3,763 engine applications; Y8V `b4fae877c35604fe`, 3XD `74db6e4bad736fec`) and the
constructed certification game is identical but for the pin. Matrix: clauses **59 / 3 / 0 / 0 / 0**; interactions and
invariants **25 / 2 / 0 / 0 / 0** — **S10-21 / X14 CLOSED — SUPERSEDED** (see S10-21). Every owner decision re-verified
with durable evidence (certification document §Q.5); none re-opened, none new. **What this closure does NOT certify or
close:** Delayed Auction (still `DEFERRED — PRE-LAUNCH VARIANT CERTIFICATION REQUIRED`); and the variant's certification
is not the repository's backlog — these stay open where they are routed, none a certification blocker: **S10-11** (the
unpinned Firestore-era path; AWS / live multiplayer), **S10-27** (the derived $0 withhold; a later derived-action pass),
**U-41** and **U-43** (statistics residuals; the dedicated statistics / residual pass), **OD-UR-9 / UR-F16** (the hosted
cryptographic seed source; AWS / live multiplayer, D-43), **UR-F15** (INFO), and the "needs a harness" rows UR-N9 / X7,
...
Unpredictable Revenue combination.)* *(UR-8, 2026-09-25: satisfied for **Unpredictable Revenue and the Gentle Rust +
Unpredictable Revenue combination**, below. It still binds **Delayed Auction**.)*
...
> | S10-21 | a completed Yellow Sign / Unpredictable Revenue fixture restoring the displaced coverage | **Variant Certification — Unpredictable Revenue** (a concrete deliverable) *(UR-8, 2026-09-25: **CLOSED — SUPERSEDED** by the UR-7 constructed certification game — see S10-21)* | from legal play under the current engine, or a deliberately constructed legal test game with stated provenance — never a hand-edited historical log |
...
**Slice 10.3 + 10.3b (2026-09-22, #1690, #1691): `RESOLVED` (uncommitted, owner gate).** One builder, `gameEngine/actionContext.ts`:
`sandboxActionContext(providers, {state, msg, actor, grid, gridBefore})` builds the reducer's whole context (chart
injections, author, grid, era, market geometry, par box, the message's par, lay geometry) and
`layAuthorityContext(providers, state, gridBefore)` the `LayTile` grid step's; `RoomEngine.apply` and `App.tsx`'s dispatch
both call them — the shell over `SHELL_PROVIDERS = sandboxReplayProviders()`, the engine's own provider set. The shell
supplies only which board (`before` + the chart/auction mirrors), which grids (`mapGridRef` after the lay; the pre-lay
snapshot) and the author. The parity inventory found ONE semantic difference: the shell handed the chart step
`isCarcosanSale` (mark-only) and the providers never did — so the **server never charged the Blood Price** (#1090) that
every browser charged, and a browser charged it even for a Carcosan sale the core then refused. `isCarcosanSale` (with
`certificatesSold`) is now the reducer's own predicate (`chartStepContext`: the gilding AND `trainSaleRefusal` at
settlement), absent from `SandboxMarketContextInjection` by construction. Measured corpus-neutral: 0 Carcosan transfers
among the 11 stored `BuyTrainFromCorporation` entries in the 18 files (S9-11's sweep, re-run). The shell's market
sentence is `sandboxChartStepReport` — the reducer's own chart step (board gates, auction step, context) on the board and
context the reducer receives — replacing a report that asked `shareSaleBlock` (not `stockSaleRefusal`), no holds and no
double-certificate count (it narrated two sales the reducer refused, server/Z6C 388 and 479, both Operating-Round
`SellStock`s). All-pass regression: `stage103CompositionCoupling.test.ts` A (a pinned room played through
`RoomSession`, replayed by `replayLog` under `SERVER_REPLAY_POLICY`: owner +income, Bank −income, money conserved,
replay == live). Canonical 18/18 unchanged (below, Part E). Version stays **7**; the Stage-10 closure bump **7 → 8**
remains owed and its changelog row should name the server's Blood Price.
**Slice 10.3b (2026-09-22, #1691): chart/core atomicity — the condition for `RESOLVED`.** The chart step runs before
the core (#272/#273) and relied on a hand-kept list of pre-chart refusals; the audit found the list short. Reproduced
on the 10.3 tree: a `DeclareDividends` whose stated amount is not what the trains ran (`dividendAmountRefusal`, a core
gate `dividendRefused` does not ask) walked the token and — mid-round — re-sorted the operating queue on the moved price,
money and cursor untouched; an author-less `SellStock` (`saleRefused` answers `false` without a seller) walked the token
for any sale the core's `stockSaleRefusal` then refused (Operating Round, first Stock Round, non-certificate bundle,
unstarted corporation). Both were refused at the live ingress (`turnRefusal` asks `dividendAmountRefusal`; the transport
always carries an author), so live-unreachable through the server, but reachable in replay, solo play and the shell. The
Blood Price's pre-chart `trainSaleRefusal` already covered every constructible transfer refusal (7 cases pinned); only the
D-6 funded-buyer arm path (a ledger refusal inside the arm) was theoretically outside it. Fix, one boundary for every
family: `marketTransaction` — the core is judged on the moved board exactly as before (every accepted action
bit-identical), and if it declines (identity, or content-unchanged for an arm's own refusal) the chart move is
discarded and the refusal is the core's refusal of the UNMOVED board (#1596's offer retirement kept).
`sandboxChartStepReport` reads the same transaction, so the shell's sentence is `null` for every declined action.
Corpus: 109 stored chart moves, 0 declined by the core (measured before the change); 18/18 unchanged after it.
`stage103bChartCoreAtomicity.test.ts` (15). Filed for the UI sweep, not changed: the shell's sold-out-rise sentence reads
`marketMarkForCompany` after the mirror already holds the post-rise chart, so its "rose from" may quote the risen price
(narration only; unverified).
...
**Unpredictable Revenue residual (UR-3, 2026-09-24).** A Firestore room deals unpinned, so it keeps the Yellow Sign's
legacy request path (the development corpus's stored entries need it byte for byte): the shell still narrates the Sign
from the drain and dispatches the request (refused there by #1407's catch-up guard — UR-F1), a crafted client could still
send, delay, aim or omit one (UR-F2), a standard-table request meets no variant gate (UR-F3), and the legacy Mark's
narration judges the pre-settlement fleet (UR-F5 / UR-F6). The fog (OD-UR-2), the phase (OD-UR-3) and the gilded
trade-in (OD-UR-7) fixes apply there too. *(OD-UR-13, D-46:)* the legacy Mark writes no removal record — the taken train
simply leaves the fleet, as it always did — so on an unpinned board UR-F19's phantom stock and phase regression remain
possible, reachable only through a crafted request or a stored legacy Mark (the normal client's request is still refused
by the drain's catch-up guard; the corpus shows none: 0 phase decreases in 18 files); writing the record there would
change what a stored unpinned Mark replays to. Retiring the path closes the rest; no shared automatic resolution was built, because it would
need a discriminator the stored corpus does not carry.
**UR-7 (2026-09-25) — the host's Yellow Sign debug chip (UR-N62).** The chip, its cycle and Ctrl+Shift+Y (#1128) now
appear only where their force can act — an unpinned Unpredictable Revenue board (`forcedSignToolInForce`, `yellowSign.ts`);
on a pinned table the chip is gone (it was inert there, but its tooltips named the Sign's phase windows — D-42). The
unpinned path keeps #1128's tool until this item retires it. **Observation, recorded (not changed):** the game server's
room document accepts the `forced-sign` op from any member, not only the host (`server/src/gameServer.ts`, the
`"forced-sign"` case — the `"kick"` case checks the host); the flag is not game state, it is inert on every pinned table,
and only the unpinned path's narration reads it. Retire it with the Firestore-era path, or check the host there if the
unpinned tool outlives this item. Still `DEFERRED`.
...
`mintSeed`).
**UR-7 (2026-09-25): still `OPEN` — NOT built in UR-7.** UR-7 built the constructed certification game (G1, two tails,
+ G0 — `utils/unpredictableRevenueCertificationGame.test.ts`), which restores part of the displaced evidence live —
Carcosan Railways (C&O's president) and The Redeemer (the Blood Price's buyer in one tail, nobody in the other), the
Gravedigger and the Rust Belt in dollars (exact), a phase-D timeline of more than ten Operating Rounds *(UR-8: ten —
OR 3.1 – 6.3 — and not asserted as a count)* — but it does not
reach the bank's end, and the displaced assertions in `accolades.test.ts`, `roundReplay.test.ts` and
`gameHistory.test.ts` are not restored (no Bagholder / Little Engine / Farmhand detail on a completed game). A fixture
committed now would be dealt and pinned at 9 and refused after UR-8's 9 → 10 boundary; a generated completed game
(tail A continued with the short bank, from the constructed start and the script) is the natural shape at v10.
**Routed: UR-8 or a named follow-up — the planner's call.** Category D kept (test substrate, not a rule).
**UR-8 (2026-09-25): `CLOSED — SUPERSEDED` by the UR-7 constructed certification game.** The item existed to give
Unpredictable Revenue's rare events live, completed-game coverage once JUNO-Z6C stopped reaching one; it was classified
"not a closure blocker; no rule depends on it", and no project policy requires a completed-game capture for certification
(GR-5 certified Gentle Rust without one). The constructed game (`utils/unpredictableRevenueCertificationGame.test.ts`) is
stronger evidence for every Unpredictable Revenue assertion S10-21 carried: played through `RoomSession.submit` on the
current authority, pinned, replayed under `SERVER_REPLAY_POLICY`, deterministic, with a paired standard control — and it
asserts exactly what S10-21 could only assert as non-null: Carcosan Railways (both tails), The Redeemer (tail B's buyer;
nobody in tail A), the Mark (award, removal, nullified route), the Gravedigger ($1,380) and the Rust Belt ($940) with the
Sign's takings excluded, per-train / per-corporation statistics and the phase carried to D; "Marked by an Outer God" is pinned
by `yellowSignRunBoundStats` and The Cowboy by `unpredictableRevenueStats`. A capture committed in UR-7 would have been
dealt at 9 and refused at the 9 → 10 boundary. **Non-UR remainder, recorded (not a blocker, not a new fixture):** the
Bagholder and Little Engine detail-format lines in `accolades.test.ts` stay vacuous on CV4, and the phase-5+,
more-than-ten-OR timeline in `roundReplay.test.ts` / `gameHistory.test.ts` stays on CV4's shorter timeline — generic
epilogue coverage a standard completed game would supply; it belongs with U-41 / U-43's dedicated statistics / residual
pass (cross-referenced from U-43). *(Correction to UR-7's certification-document §O: the constructed game does not assert a
Bagholder detail.)* No
fixture built, no historical log touched, nothing repinned. Evidence: certification document §Q.4; UR audit rev 12.
...
**S10-27. The room derives a forced $0 withhold for a corporation left trainless at Dividends whose run still earned.**
Status `OPEN` (found by UR-3, 2026-09-24; not fixed — outside UR-3's scope). `nextDerivedAction` (`derivedActions.ts`,
#292 / #1275) treats "no earnable revenue" at Dividends as `skipReasonFor(earnableRevenueVerdict(..))`, which answers from
the fleet: a corporation with no trains gets `DeclareDividends { revenue_amount: "0", distribute: false }` even when its
run this turn has already priced a non-zero `last_route_revenue`. The reducer refuses that declaration (C1,
`dividendAmountRefusal`: "$0 does not match the run"), and — derived no-ops still append by design (#1685 / 10.2) — the
room commits a no-op derived entry and spends the turn's guard key; the president then declares the real figure as usual,
so no board or money is wrong. **Reproducible with Gentle Rust alone** (a corporation whose only train is on its Final Run
runs, the train is retired at Run → Dividends, the corporation reaches Dividends trainless with its run's revenue) and
newly reachable with Unpredictable Revenue (a Mark that takes the corporation's last train). Repair (a later Stage-10 /
derived-action pass): at Dividends, force the withhold only when nothing ran (`routes_run_this_turn === 0` and a zero
run), i.e. drop the fleet-based answer once the run is behind the corporation. Replay: none for stored logs (the entry
is a no-op either way); new logs would stop recording the no-op. Evidence: `yellowSignRunBound.test.ts` ("hosted: the
run's own entry applies the same Mark") pins that anything after the run is the room's own derived step.
**UR-7 (2026-09-25): reproduced live, still `OPEN`.** The Unpredictable Revenue certification game meets it on its
hosted path: C&O's Gentle Rust grace turn retires its Final Run 2 and the Mark takes its 4, so C&O reaches Dividends
...
2026-09-25, uncommitted, awaiting the owner's gate — below)*. UR-F21 is **LEGALITY
SYNC / NEW ACTION**: the authority now refuses an unnamed sale of a model the seller holds both gold-trimmed and ordinary,
so the Buy Trains roster marks the gold-trimmed badge (`gildedSalePositions`, the chips' multiset order) and each badge
sends its own copy (`gilded: true` / `false`) — only when the seller holds a gilded copy of that model, otherwise the
unnamed sale, byte for byte; the shell's proposal, direct buy and pending-offer view carry it (sandbox / rooms only — the
contract path never names a copy). UR-F22 is **STATE VISIBILITY**: the warning names the BUYING corporation's drop and
shows only for the gilded copy; the consent prompt tells the seller which copy it answers for and, for the gilded one, the
buyer's drop and the seller's release. Evidence: `components/carcosaBloodPricePanel.test.tsx` (rendered roster, warning,
dispatched copy; the prompt; source pins on `App.tsx`), `batch60` pins. **Still owed (UR-6, copy only):** the Activity
Log's offer / trade lines name the model, not the copy; the Rules Reference's Blood Price text (UR-F13). **No reducer
change beyond UR-4's own, no replay effect from the UI, no golden.** *(UR-6, 2026-09-25 — **`RESOLVED`**, STATE VISIBILITY: the
offer, answer and trade lines name the copy through the authority's own predicates (`utils/saleCopyDisclosure.ts` →
`isCarcosanTransfer`, `resolveSaleCopy`) — the gold-trimmed copy's lines say Blood Price, paid by the buyer, the train
cured; an ordinary copy beside it says "not the gold-trimmed one, so no Blood Price"; every other sale reads byte for byte
...
STATISTICS only.** (1) **The Salvager counts a Bank Pool purchase as a trade-in.** `BuyHardwareFromPool` naming
`returned_model_type` is the Bank Pool purchase (#1314, `buyReturnedTrain`), but the history reads that field as a
trade-in's returned model: it bumps `tradeIns` (The Salvager, "traded in the most trains") and wires the model into the
fleet-loss fates as a trade-in's (dormant there: a pool purchase takes nothing off the buyer's roster). The Diesel exchange (`ExchangeTrainForDiesel`) is the only trade-in. (2) **A refused `RunMultipleRoutes` is booked**
at the figures the board still holds: harmless in every corpus case (each is a turn's first message, $0 — JUNO-FCJ 95,
JUNO-Z6C 52, JUNO-3XD 28), but a refused duplicate mid-turn would book the turn twice. (3) **Legacy `RunManualRoute`
runs are not booked at all** (JUNO-Y8V's 92 give it no lifetime revenue). (4) **The Cowboy skips a pre-#1051 seedless
Unpredictable Revenue run**, whose line the shell printed off `legacyTurnSeed`. *Candidate resolution — pending its own
review, NOT ratified here:* (1) count only `ExchangeTrainForDiesel`; (2) book only accepted runs (keep the Map's key
order — the accolades' tie order depends on it); (3) book `RunManualRoute` like a one-route run; (4) fall back to
`legacyTurnSeed`. Each changes standard-game statistics, so each needs owner review and tests; UR-5 changes none of them
and pins none of them. **Deferred to the dedicated statistics / residual pass** (with U-41). **No reducer change, no replay
effect, no golden.** Source: UR audit "UR-5 implementation (rev 9)", backlog S9-7 UR-5 block. *(Cross-reference, UR-8,
...
still **NOT certified** — it is implemented and certified with Unpredictable Revenue (UR-3 … UR-8). *(Implemented in
UR-3, 2026-09-24 — committed and pushed as `9d0cf3a`: `settleRunYellowSign` resolves the Mark after `settleOperatingCursor`, on the unreprieved
copies, and `runWithoutTrain` attributes the nullified route on the fleet as it ran.)* `OWNER DECISION`
...
*(UR-8, 2026-09-25: combined Gentle Rust + Unpredictable Revenue is **CERTIFIED** with Unpredictable Revenue at
`RULES_ENGINE_VERSION` 10; this ruling is changelog row 10 (2).)*

*(UR-8, 2026-09-25 — applies to D-37 … D-51 below.)* Every "replay-semantic (the 9 → 10 boundary)" / "owed to row 10"
note in these entries is now **carried by `RULES_ENGINE_VERSION` 10**, changelog row 10: D-37 → (1), D-36 → (2), D-38 →
(3), D-39 → (4), D-41 → (5), D-48 and D-50 → (6), D-47 → (7), D-46 → (8). D-40 (the minted award), D-42 (the Rules
Reference), D-43 (the seed source — implementation still deferred to AWS / live multiplayer), D-44 (the undo), D-45 (the
name), D-49 and D-51 (the statistics basis) are named in the row as NOT rules. Unpredictable Revenue is CERTIFIED (S9-7,
"Variant Certification 1B — CLOSED"). The entries are otherwise kept as written.
...
of S9-1 / #1661 for pinned tables and closes the architectural cause of UR-F1 and UR-F2. **Implemented in UR-3
(2026-09-24; committed and pushed as `9d0cf3a`)** — `settleRunYellowSign`, `yellowSignRequestRefusal`, the record `last_run_yellow_sign`;
unpinned boards keep the request path; replay-semantic (the Unpredictable Revenue 9 → 10 boundary). `OWNER DECISION` (variant specification). Source:
...
(UR-F18). A boundary notice may carry the narration later (UR-6). *(UR-6, 2026-09-25, uncommitted: the fog's ruled sound
and film (#1092 / #1093) now play at the set boundary, once, replay-guarded; the Activity Log line is the durable notice;
no modal.)* **Implemented in UR-3 (2026-09-24; committed and pushed as `9d0cf3a`)** —
`fogAtSetEnd` in `settleRoundTransitions`' end-of-set branch (every exit), a minimal Activity Log line; replay-semantic. `OWNER DECISION` (variant specification). Source: UR audit "Owner rulings (rev 2)"; S9-2 and
S9-3 annotated.
...
changes nothing beyond the recipient's own fleet; a synthetic D is still not a real D purchase. **Implemented in UR-3
(2026-09-24, by the owner's brief; committed and pushed as `9d0cf3a`)** — `derivePhase` skips ghost-accounted copies; replay-semantic. `OWNER DECISION` (variant specification). Source: UR audit "Owner rulings (rev 2)".
...
design, not decided here). **Implemented in UR-3 (2026-09-24, by the owner's brief; committed and pushed as `9d0cf3a`)** —
`exchangeableTrains` subtracts the gilding multiset, `gildedExchangeReason` says why; replay-semantic.
...
train-limit treatment, its eventual disappearance, and the Blood Price's consequences once OD-UR-5 is decided *(since: (a) and (c) decided — D-48; (b) decided — D-50: OD-UR-5 is fully decided)* — but not
the Yellow Sign's hidden trigger conditions or odds merely for completeness. Copy only; UR-6, after OD-UR-5 (UR-F13). *(Implemented in UR-6, 2026-09-25, uncommitted — UR-F13 `RESOLVED`; the tie
sentence waits for UR-7, UR-F20.)*
...
"Unpredictable Routes" copy (host setup, room list) is normalized to it. Copy only; UR-6 (UR-F14). *(Implemented in UR-6, 2026-09-25, uncommitted — UR-F14 `RESOLVED`.)* `OWNER DECISION`
(presentation). Source: UR audit "Owner rulings (rev 2)".

**D-46. Unpredictable Revenue: the Mark's taken train is permanently removed from the game, and phase progression is
monotonic (OD-UR-13 — DECIDED 2026-09-24, during UR-3).** Ruled: "A train taken by the Yellow Sign Mark is PERMANENTLY
REMOVED FROM THE GAME. It does NOT: return to the depot; enter the Bank Pool; become purchasable again; increase available
depot stock. The corporation still receives the already-decided Mark award under OD-UR-4." And the invariant: "PHASE
PROGRESSION IS MONOTONIC. Once a phase has been reached by the qualifying REAL train purchase, later removal of trains
cannot lower the phase, reopen an earlier train tier, undo rust, undo Gentle Rust effects, close a depot shelf, or reverse
an 18XX+ era transition" — regardless of the removal mechanism (the Yellow Sign Mark, ordinary rust, Gentle Rust
destruction, fog, discard, any other). For UR-3 the Mark is fixed narrowly: a durable authoritative removed-from-game
representation sufficient for replay and depot accounting, no silent reappearance as depot stock, the phase kept,
replay reproducing the removal, statistics still classifying the train `taken`; no Bank Pool destination invented; a
broader pre-existing phase-regression defect outside the Mark path to be reported, not fixed in UR-3. **Implemented in
UR-3 (2026-09-24; committed and pushed as `9d0cf3a`)** — `removed_trains` written by the run-bound Mark (pinned Unpredictable Revenue tables);
`derivePhase` / `depotInventory` read it; the other removal paths were checked and none regresses the phase (UR audit,
"UR-3 implementation (rev 3)"). Resolves UR-F19. Replay-semantic for the 9 → 10 boundary (UR-8, Part E). `OWNER DECISION`
(variant specification). Source: the owner's ruling during UR-3 (UR audit, "Owner rulings" — OD-UR-13 row).

**D-47. Unpredictable Revenue: an exact $5 tie of the modified revenue rounds toward the printed revenue (OD-UR-10 —
DECIDED 2026-09-24, option 10-C).** Ruled: "when the modified Unpredictable Revenue amount lands exactly halfway between
two $10 increments, round toward the corporation's original printed revenue." If the exact modified amount is not a $5
tie it still rounds normally to the nearest $10; if it IS an exact $5 tie, the adjacent $10 value closer to the original
printed revenue is chosen. Examples: printed $50 — −10% = $45 → $50, +10% = $55 → $50; printed $150 — $135 → $140,
$165 → $160; printed $250 — $225 → $230, $275 → $270. The owner's rationale: damp the die's volatility at low revenue
values; stabilize the effect at higher values; preserve symmetry around the printed revenue; eliminate the current
half-up upward bias. It replaces the implementation's ordinary half-up rounding of exact ties (`roundToTen`,
`gameVariants.ts`). Every printed value is a multiple of $10, so only the ±10% faces can tie (at printed ≡ $50 mod $100)
and only the +10% tie changes — the −10% tie already rounds toward printed — which makes the six-face mean exactly the
printed figure (UR audit §8.2, rev 4). **Implementation pending:** UR-F20, routed to UR-7 (the rounding table and the tie
rule); a gameplay / replay-semantic change owed to the deliberate 9 → 10 boundary (UR-8, Part E row 10); the unpinned
corpus must keep replaying as it was played (X13 / D-9). Not certified until implemented and tested. `OWNER DECISION`
(variant specification). Source: the owner's ruling (UR audit rev 4, "Owner rulings" — OD-UR-10 row).
*(UR-7, 2026-09-25 — **implemented, uncommitted, awaiting the owner's gate**; UR-F20 `RESOLVED`.)* `rollTurnRevenue`
judges the tie on the exact product (`printed × percent ≡ 500 mod 1000`) and sends it to `roundRevenueTowardPrinted`
(`gameVariants.ts`); every other amount still goes through `roundToTen` unchanged. Pinned: every example above, the whole
table ($10 … $1,000 × six faces) and the whole integer domain against an independent oracle, the reducer's run, the
Dividends declaration, the payout and withhold, the hosted draw, replay, restore and undo (`revenueTieRounding`), and
the tie on a hosted constructed game (the certification game §1). The Rules Reference says it. The corpus replays
unchanged (0 exact ties among 247 Unpredictable Revenue runs). Implemented at v9 on every table; owed to row 10 (UR-8).

**D-48. Unpredictable Revenue: the Blood Price — the buyer's train is ORDINARY (OD-UR-5(a) = 5a-1) and a sale names the
copy (OD-UR-5(c) = 5c-2) — DECIDED 2026-09-24, (a) clarified by the owner the same day; OD-UR-5(b) decided later the same day (D-50).** **(a)**
A train acquired through the Blood Price becomes an ORDINARY train for the buyer: the supernatural / gilded / Carcosan
status is cured by the Blood Price. It is an **additional ordinary train**: the synthetic Carcosa gift becomes one more
ordinary train in circulation — effectively +1 relative to the printed depot supply — and NEVER a special class of train
or one that keeps any special exemption (the owner's informal "bonus train" means only this). After the acquisition it
counts against the buyer's train limit, runs normally, rusts normally, can later be sold normally, can be a Diesel
trade-in if otherwise eligible, and has no fog deadline, no gilding, no Carcosan train-limit exemption and no other
supernatural status. This is #1090's rule and the implementation's behaviour today (UR audit: GHL E); S9-3's "remains a
ghost train and STILL disappears on the same D-triggered schedule" is superseded. *(Owner clarification, 2026-09-24 — it
closes the phase question first recorded here for UR-4; no owner decision remains on it, and no new gameplay exception
is required:)* the gift is the lowest-value train the authoritative Depot rule supplies (S9-2); the cured train keeps
only the provenance / accounting marker needed to remember that it originated synthetically rather than consuming one of
the printed Depot copies, and that provenance means only the +1 — **synthetic origin / supply provenance may persist;
supernatural rules status does not** (it ends at the Blood Price). The Blood Price changes no phase for the base game's
reason: it is an **intercorporate train purchase, not a purchase from the Bank / Depot**, and the phase change, rust and
analogous first-train consequences are triggered by the qualifying Depot purchase — so it advances no phase, triggers no
rust, creates no Gentle Rust marks, opens no new Depot tier, advances no 18XX+ era and does not start the real-D doom
trigger merely because the train is a D. That is not because the cured train stays supernatural, and it is not a special
rule that ignores the cured train for its provenance. **For UR-4 (an implementation note — nothing designed or built
here):** today one marker, `ghost_trains` (#1673's synthetic provenance), is read by the depot tally (the +1), by the
phase (D-39) and by the real-D check; UR-4 keeps the required accounting and makes sure the cured train is otherwise
treated as ordinary — keeping one marker if its semantics already represent this correctly, or separating the concepts
if necessary; the representation is not prescribed (UR audit §12.2, OD-UR-5). **(c)** If the selling corporation owns a
gilded copy and an ordinary copy of model X, the sale must distinguish which copy is sold: selling the ordinary copy
does NOT trigger the Blood Price and does not purify or remove the gilded copy; selling the gilded copy DOES trigger the
Blood Price. Not implemented — **UR-F21** (the transfer is model-level today), UR-4; replay-semantic, owed to the 9 → 10
boundary. *(Implemented 2026-09-25 — UR-4, owner-gated, committed and pushed as `a6183b5`: (a) pinned live with the +1 carried
through the Bank Pool; (c) UR-F21 `RESOLVED`.)* **(b)** — whose stock marker takes the Left 1 / Down 1 — is **NOT decided**: the owner currently leans toward
the buyer ("the buyer pays the Blood Price") and still goes back and forth between buyer and seller; that is context,
not a ruling, and the implementation stays #1090's (the seller) until the owner rules. *(Since decided the same day — **D-50**: the BUYER's marker moves Left 1 / Down 1; the seller's
does not.)* `OWNER DECISION` (variant
specification). Source: the owner's rulings (UR audit rev 5, "Owner rulings" — OD-UR-5 row) and the owner's
clarification of (a) (UR audit rev 6, §12.2).

**D-49. Unpredictable Revenue: the statistics' basis (OD-UR-6 — DECIDED 2026-09-24; 6.1 decided in principle, its per-train question the same day — D-51).**
**6.1** Revenue statistics reflect the revenue actually paid after the Unpredictable Revenue die adjustment, not merely
the printed pre-die total; corporation / turn-level revenue statistics (The Workhorse and lifetime revenue, The
Juggernaut, the Revenue-per-OR chart, White Elephant, Little Engine) use the paid figure. The die modifies the
corporation's whole turn total, so a statistic that needs a per-train adjusted value (Master of the Line, the fleet
ledger's "earned") has no canonical allocation: ~~that allocation is an **isolated open sub-question of 6.1** — neither
the printed figure nor a proportional split is adopted by default~~ *(since decided — **D-51**: a per-train statistic uses
the printed value of the train's successfully completed route — not an allocation of the paid total)*. **6.2** A train whose route the Mark nullified does
not count as having earned that revenue — "the train disappeared instead of completing the run; it never made it to the
station." **6.3** A synthetic Carcosa gift is NOT a purchase: it counts for no purchase-count statistic, no "bought a
Diesel" statistic and no purchase-based accolade (The Early Adopter and analogous purchase-derived awards). A later
acquisition of the formerly gilded train by another corporation through the Blood Price IS a genuine train purchase by
the buyer and receives ordinary purchase-based statistical / accolade treatment — Diesel-purchase treatment included,
when the train is a Diesel and the award otherwise qualifies (it follows D-48(a)). Not implemented — UR-5 (UR-F8) *(implemented
2026-09-25 — UR-5, uncommitted, awaiting the owner's gate: UR-F8 `RESOLVED`)*;
derived history only, no version effect. `OWNER DECISION` (statistics). Source: the owner's rulings (UR audit rev 5,
"Owner rulings" — OD-UR-6 row).

**D-50. Unpredictable Revenue: the Blood Price's market consequence falls on the BUYER only (OD-UR-5(b) — DECIDED
2026-09-24).** When a corporation buys the gilded train through the Blood Price, the buyer pays the cash price and the
**buyer's** stock marker moves Left 1 / Down 1; the seller gets **no** separate stock-price movement — the seller's
benefit is its release from the Carcosan curse / supernatural burden (it is absolved: `is_carcosan` cleared, the gilding
burned, the doom clock cleared). The market penalty is **never** applied to both corporations. This follows S9-3's "The
purchasing corporation pays the required Blood Price consequences (cash and corporate stock-price consequence …)" and
**supersedes #1090's "Left 1, Down 1 market movement for the selling corporation"**, which the implementation carries
today. **Do not restore the seller move** — not from #1090, not from the market arm's design note ("THE SELLER MOVES,
NOT THE BUYER. The toll is for letting the thing go.", `sandboxSession.ts:2679`), not from the Train Purchase panel's
warning ("The selling corporation's share price will immediately drop", `TrainPurchasePanel.tsx:1388–1389`), not from
the narration's design note ("[Selling Corp]'s stock dropped", `App.tsx:6904`) and not from the suites that pin it
(`bloodPriceArrival`, `stage103CompositionCoupling`): all describe the superseded rule. The owner's earlier lean toward
the buyer (D-48) is now the ruling. With (a) (D-48, clarified the same day) and (c) (D-48), **OD-UR-5 is fully
decided**. Not implemented — **UR-F22** (UR-4; the copy with UR-6); replay-semantic, owed to the 9 → 10 boundary.
*(Implemented 2026-09-25 — UR-4, owner-gated, committed and pushed as `a6183b5`: UR-F22 `RESOLVED` — the buyer's marker moves, the
seller's never; the warning and the prompt name the buyer.)*
`OWNER DECISION` (variant specification). Source: the owner's ruling (UR audit rev 7, "Owner rulings" — OD-UR-5 row).

**D-51. Unpredictable Revenue: per-train statistics use the printed value of the train's completed route (OD-UR-6.1 —
the per-train question DECIDED 2026-09-24).** The sub-question D-49 isolated is closed. **Corporation / turn-level
revenue statistics** use the ACTUAL PAID revenue after the Unpredictable Revenue die adjustment (D-49). **Individual
train / route statistics** use the PRINTED value of that train's successfully completed route. The reason: the die
modifies the corporation's whole turn total; there is no canonical adjusted dollar amount belonging to each individual
train; the printed route value is the actual route-specific quantity. The per-train figure is **not an allocation of the
paid total** — no proportional allocation, no equal allocation and no die-adjusted per-train value is invented, and the
per-train figures are not expected to add up to the paid turn total. With 6.2 (D-49): a successfully completed route
contributes its printed value to the per-train statistics; a Mark-nullified route contributes nothing — the train
disappeared instead of completing the run. 6.3 stands (D-49): a synthetic Carcosa gift is not a purchase; a Blood Price
acquisition is a genuine purchase by the buyer. So Master of the Line and the fleet ledger's "earned" read each
completed route's printed figure (never the nullified route), while The Workhorse and lifetime revenue, The Juggernaut,
the Revenue-per-OR chart, White Elephant and Little Engine read the paid turn total. **OD-UR-6 is fully decided.** Not
...
| 9 (owed: **10** at Unpredictable Revenue closure, UR-8) | **UR-3** (2026-09-24 — owner-gated, committed and pushed as `9d0cf3a`) | **The Yellow Sign's authority and the Carcosa rules it made reachable (OD-UR-1, OD-GR-3, OD-UR-2, OD-UR-3, OD-UR-7, OD-UR-13).** On a pinned Unpredictable Revenue table: (1) the stage is resolved and applied inside the accepted run's own entry (`settleRunYellowSign`, after the Run → Dividends settlement), recorded as `last_run_yellow_sign`, and a client `YellowSignEvent` is refused; (2) the Mark judges the post-settlement fleet — never a Final Run train — and nullifies the taken train's own route; (3) the run's breakdown is the authority's pairing; (7) the Mark's taken train is recorded as removed from the game (`removed_trains`, never the Bank Pool), counted toward the phase and off the depot — the phase never falls back across it and the copy never returns to sale (D-46). On every Unpredictable Revenue table: (4) the gilded train leaves at the END of set N+1 (`fogAtSetEnd`), never on a run; on every table: (5) synthetic trains never advance the phase (`derivePhase`); (6) a gilded train is never a Diesel trade-in (`exchangeableTrains`). Unpinned boards keep the legacy request path byte for byte (no removal record there — S10-11). `RULES_ENGINE_VERSION` stays **9**. | **18-file corpus measured old (baseline scratch build) vs new (UR-3 scratch build): 4,105 stored / 3,731 applied / 374 dropped / 3,763 engine applications — 0 state / grid / cursor / derived-action / known-phase differences at every engine application; 18/18 final boards identical; the known phase never decreases in any file on either tree** (figures: UR audit §16, rev 3). Y8V 668 / 628 / 40, OR 13, `b4fae877c35604fe`; 3XD 322 / 320 / 2, 323 applications, SR 12, `74db6e4bad736fec`. The corpus carries no gilded train, no ghost and no pinned file (so no `removed_trains`), and its stored `YellowSignEvent` entries (JUNO-Z6C 203, 567) are unpinned — the branch they always took. Corpus file bytes (md5) identical. No golden, fixture, log or export touched. |
| 9 (owed: **10** at Unpredictable Revenue closure, UR-8) | **UR-4** (2026-09-25 — owner-gated, committed and pushed as `a6183b5`) | **The Blood Price (OD-UR-5, D-48 / D-50), on every table.** (1) The sale names the copy: optional `gilded` on `BuyTrainFromCorporation` / `ProposeTrainPurchase` (so on the offer and its derived settlement); an unnamed sale of a model the seller holds both gold-trimmed and ordinary is refused; only the gilded copy's sale is the Blood Price (the gilding burns, the curse and clock clear, the chart moves); provenance leaves with the copy that has it (UR-F21). (2) The Blood Price moves the BUYER's marker Left 1 / Down 1, never the seller's (UR-F22). (3) An additional (cured) copy entering the Bank Pool keeps its provenance there (`returned_ghost_trains`): the Depot tally, the phase and the real-D check do not count it as printed stock or a Depot purchase, and a pool purchase hands the provenance to the buyer. NOT rules: The Redeemer's reading of the copy (derived statistics), the panel's copy choice, the warning and the prompt (UI). `RULES_ENGINE_VERSION` stays **9**. | **18-file corpus measured old (baseline scratch build) vs new (UR-4 scratch build): 4,105 stored / 3,731 applied / 374 dropped / 3,763 engine applications — 0 state / grid / cursor / derived-action / known-phase differences at every engine application; 18/18 final boards identical; the known phase never decreases.** Y8V 668 / 628 / 40, OR 13, `b4fae877c35604fe`; 3XD 322 / 320 / 2, 323 applications, SR 12, `74db6e4bad736fec`. The corpus carries no gilding, no provenance marker, no pool provenance and no Carcosan transfer (11 stored `BuyTrainFromCorporation`, 0 Carcosan), so no stored entry is reinterpreted; the new field is written on no corpus board. Corpus file bytes (md5) identical. No golden, fixture, log or export touched. |
| 9 (owed: **10** at Unpredictable Revenue closure, UR-8) | **UR-7** (2026-09-25 — owner-gated, committed and pushed as `ff7a04b`) | **The exact-$5 tie of the modified revenue (OD-UR-10 = 10-C, D-47), on every table.** `rollTurnRevenue` judges the tie on the exact product (`printed × percent ≡ 500 mod 1000`) and rounds it toward the printed total (`roundRevenueTowardPrinted`); every other amount rounds half up to the nearest $10 exactly as before. On printed multiples of $10 only the +10% face at printed ≡ $50 (mod $100) pays differently — $10 less than before — so a stored run that met such a tie would replay at the lower figure and its stored declaration of the old one would be refused. NOT rules: the Rules Reference's tie sentence, the debug chip's visibility (UR-N62 — presentation; the pinned authority unchanged), the certification tests, the constructed game and the documents. `RULES_ENGINE_VERSION` stays **9**; **do not deploy to a server holding live v9 rooms before UR-8**. | **18-file corpus measured old (the UR-6 tree) vs new (the UR-7 tree): 4,105 stored / 3,731 applied / 374 dropped / 3,763 engine applications — 0 state / grid / cursor / derived-action / known-phase differences at every engine application; 18/18 final boards identical.** Y8V 668 / 628 / 40, OR 13, `b4fae877c35604fe`; 3XD 322 / 320 / 2, 323 applications, SR 12, `74db6e4bad736fec`. The corpus holds 247 Unpredictable Revenue runs and **0 exact ties** (one printed total ≡ $50 mod $100, at face 4), so no stored entry is reinterpreted. Corpus file bytes (md5) identical. No golden, fixture, log or export touched. |
| **10** | **UR-8 — Unpredictable Revenue certification closure** (uncommitted, 2026-09-25) | **The bump.** `RULES_ENGINE_VERSION` **9 → 10**, `SUPPORTED_RULES_ENGINE_VERSIONS` derived `[10]`, changelog row 10 naming exactly eight replay semantics — (1) UR-3 (OD-UR-1, D-37): on a pinned Unpredictable Revenue table the Yellow Sign is an automatic consequence of the accepted run, applied in the run's own entry (`settleRunYellowSign`, `last_run_yellow_sign`), and on every pinned table a client `YellowSignEvent` is refused at ingress and in the reducer; (2) UR-3 (OD-GR-3, D-36): the Mark judges the post-settlement fleet — a Final Run train never a candidate — and nullifies only the taken train's route; (3) UR-3 (OD-UR-2, D-38): the gold-trimmed train leaves at the END of OR set N+1, at the boundary (`fogAtSetEnd`); (4) UR-3 (OD-UR-3, D-39), every table: a synthetic train never advances the phase (`derivePhase`); (5) UR-3 (OD-UR-7, D-41), every table: no gilded Diesel trade-in ($800, LPF $750); (6) UR-4 (OD-UR-5, D-48 / D-50), every table: the Blood Price names the copy (optional `gilded`; an unnamed ambiguous sale refused), moves the BUYER Left 1 / Down 1 and never the seller, and cures into an ordinary additional train whose synthetic origin is supply provenance only, through the Bank Pool too (`returned_ghost_trains`); (7) UR-7 (OD-UR-10 = 10-C, D-47), every table: an exact $5 tie rounds toward the printed total (`roundRevenueTowardPrinted`); (8) UR-3 (OD-UR-13, D-46): the Mark's train permanently removed from the game (`removed_trains`), the phase monotonic — and, separately and explicitly NOT as rules, the minted award (OD-UR-4), UR-5's statistics basis (OD-UR-6), UR-6's UI / copy / Rules Reference / naming (OD-UR-8, OD-UR-12), UR-7's tie sentence, debug-chip visibility (UR-N62) and certification evidence, the undo rule (OD-UR-11) and the seed source (OD-UR-9, deferred). None asks the pin's value (UR-3's seam asks for the pin's PRESENCE). A version-9 room is held (`incompatible`) under every policy, before the reducer sees an entry, and nothing rewrites its pin; a new deal is stamped 10 over any claim. Unpinned (Firestore-era) boards keep the legacy request path (S10-11). New `unpredictableRevenueClosure.test.ts` (19; owns the current literal); `gentleRustClosure`, `unpredictableRevenueCertificationGame`, `carcosaBloodPrice` J1, `revenueTieRounding` and `unpredictableRevenueStats` narrowed, version literals only. **Unpredictable Revenue CERTIFIED** (standalone and with Gentle Rust); S10-21 CLOSED — SUPERSEDED. | **18-file corpus measured old (the UR-7 tree, v9) vs new (the UR-8 tree, v10): 4,105 stored / 3,731 applied / 374 dropped / 3,763 engine applications — 0 state / grid / cursor / derived-action / known-phase differences at every engine application; 18/18 final boards identical; no phase decrease; all 18 files unpinned, none acquires a pin.** Y8V 668 / 628 / 40, OR 13, `b4fae877c35604fe`; 3XD 322 / 320 / 2, 323 applications, SR 12, `74db6e4bad736fec`. The constructed certification game (G1 tails A / B, G0) identical across the boundary entry for entry, final digests differing only by the pin. Corpus file bytes (md5) identical. No golden, fixture, log or export touched; nothing repinned. |
