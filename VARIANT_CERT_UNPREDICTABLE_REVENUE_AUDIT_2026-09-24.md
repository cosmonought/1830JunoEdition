...
**Rev 8 (2026-09-25) — UR-4 implemented: the Blood Price, copy by copy, paid by the buyer** *(rev 9: **COMPLETE —
owner-gated (the owner's full repository Jest gate passed), committed and pushed as `a6183b5`**; at rev 8 it was
uncommitted, awaiting that gate)*. OD-UR-5 is implemented and pinned live, on every table (pinned and unpinned — the
...
**Rev 9 (2026-09-25) — UR-4 recorded COMPLETE; UR-5 implemented: the statistics of the variant, as ruled** *(rev 10:
**COMPLETE — owner-gated (the owner's full repository Jest gate passed), committed and pushed as `0d8157e`**; at rev 9 it
was uncommitted, awaiting that gate)*. **UR-4 is COMPLETE**: owner-gated and committed as `a6183b5`
("ur4+test"), which `origin/main` also holds. **UR-5 implements OD-UR-6 (UR-F8) and UR-F9, in derived history only
(`utils/gameHistory.ts`):** a run is booked as it SETTLED — the corporation / turn statistics (lifetime revenue, The
Workhorse, The Juggernaut, the Revenue-per-OR chart, and through them White Elephant and Little Engine) on the revenue
actually paid (`last_route_revenue`, what the Dividends step declares), the train / route statistics (Master of the Line,
the fleet ledger's earned and runs) on each completed route's printed figure, never an allocation; a Mark-nullified route
contributes nothing (on a pinned table the Mark's own entry already took it out; an unpinned table's later legacy request
amends the turn's booking before any tally is drawn, and a request delayed past the Dividends step cannot rewrite what was
paid); the Carcosa gift — the one copy that arrives gilded — is acquired, not bought (no Fleet Admiral count, no Early
Adopter, $0 in the ledger, which still holds it: #1431's n is "trains held"), while the Blood Price is the buyer's genuine
purchase, a Diesel's Early Adopter included, and a cured copy bought from the Bank Pool is an ordinary purchase; The
Cowboy counts the line the table read (UR-F9). `MINTS_BY_DESIGN`'s stale comment is corrected (Appendix B item 11).
**Carcosan Railways:** its logic is correct under OD-UR-5(b) (#1421: the buyer pays and is The Redeemer; a released seller
never paid) — only its blurb is stale, routed to UR-6 (Appendix B item 13). Pre-existing, non-UR statistics residuals
recorded (backlog U-43). One new suite (46 tests: 22 fail / 24 pass on `a6183b5`), two UR-3 pins moved to the ruled basis;
focused Jest 157 suites / 3,199 tests green; tsc (frontend, server) 0; build 49 warning lines, identical to the baseline;
the 18-file corpus unchanged (0 differences in 3,763 engine applications; Y8V `b4fae877c35604fe`, 3XD `74db6e4bad736fec`),
its derived statistics identical on every standard file and moved on the four Unpredictable Revenue logs only as ruled.
`RULES_ENGINE_VERSION` stays **9** — statistics are not rules (§15). No owner decision is new or re-opened. The verdict
stays **B**; Unpredictable Revenue is NOT certified. See "UR-5 implementation (rev 9)".

**Rev 10 (2026-09-25) — UR-5 recorded COMPLETE; UR-6 implemented: the players are told what the authority does**
*(UR-6 uncommitted, awaiting the owner's full repository Jest gate)*. **UR-5 is COMPLETE**: owner-gated and committed as
`0d8157e` ("Implement Unpredictable Revenue statistics"), which `origin/main` also holds. **UR-6 is replay-neutral
presentation** — no reducer, server, message, schema, state field, digest, statistic, log, fixture or golden changed; the
engine files it touched differ only in comments (checked by transpiling both trees with comments removed). **Closed:**
**UR-F13** (OD-UR-8) — the Rules Reference states the implemented variant in two tagged blocks (the die and the Yellow
Sign's consequences in Dividends; the gold-trimmed train, the fog and the Blood Price in Buy Trains) and keeps the Yellow
Sign's trigger conditions and odds hidden; **UR-F12** — the gold-trimmed chip states the ruled lifetime; **UR-F14**
(OD-UR-12) — "Unpredictable Routes" is gone; the independent UR-4 review's **D1** — the consent prompt and the
pending-offer view disclose the Blood Price whenever the authority will SETTLE one (an unnamed offer for the seller's only
gold-trimmed copy included), through the reducer's own predicate; **U-42's** copy — the Activity Log's offer, answer and
trade lines name the copy, the Blood Price as the buyer's; Carcosan Railways' blurb (Appendix B item 13; its logic
unchanged) and the ledger's column titles (item 14); every other Appendix B item (1 – 10, 12); and UR-3's three
deferrals — the Diesel trade-in row greys a gold-trimmed copy with its reason, the fog's ruled sound and film play at
the set boundary (replay-guarded; no modal was ruled or added), and the Rules Reference says the Mark's train is gone for
good. The Redeemer's copy already named the buyer (unchanged). **Not done, by the brief:** the exact-$5 tie sentence —
the page says "rounded to the nearest $10" and no tie direction until UR-7 implements 10-C (UR-F20). Two new suites (40
tests: 25 fail / 15 pass on `0d8157e`), four updated (`carcosaBloodPricePanel`, `batch59`, `batch60`, two test comments);
focused Jest 335 suites / 6,290 tests (see "UR-6 implementation (rev 10)"); tsc (frontend, server) 0; build 49 warning
...
**Rev 12 (2026-09-25) — UR-8: UNPREDICTABLE REVENUE IS CERTIFIED at `RULES_ENGINE_VERSION` 10** — standalone and with
Gentle Rust (OD-GR-3) — subject to the owner's full-suite gate and commit. **UR-7 is COMPLETE**: owner-gated and committed
and pushed as `ff7a04b` ("Certify Unpredictable Revenue behavior"; UR-6's `cd889cb` is pushed with it — local `main` =
`origin/main`). **UR-8** took the one deliberate **9 → 10** boundary: `RULES_ENGINE_VERSION` 10, supported `[10]`, and
changelog **row 10** naming exactly the eight replay semantics of §15's "version boundary" — (1) the run-bound Sign and
its refusal of client requests, (2) the Mark on the post-settlement fleet, (3) the fog at the end of N+1, (4) synthetic
trains never the phase, (5) no gilded trade-in, (6) the Blood Price's copy, the buyer's move and the cure's provenance-only
supply tracking, (7) 10-C, (8) the Mark's train removed from the game — with the award, the statistics basis, the UI /
copy / Rules Reference, the certification evidence, the undo rule and the seed source behind a NOT RULES marker. A
v9-pinned room is held `incompatible` under every policy, never reinterpreted; new rooms deal at 10; the unpinned corpus
replays unchanged (0 differences in 3,763 engine applications; Y8V `b4fae877c35604fe`, 3XD `74db6e4bad736fec`); the
constructed certification game is identical across the boundary but for the pin. **S10-21 / X14 CLOSED — SUPERSEDED** by
the constructed certification game (its non-UR remainder — completed-game Bagholder / Little Engine formats and the long
timeline — recorded on the backlog item as generic epilogue coverage). Every owner decision re-verified; none re-opened;
no new one. Counts: clauses **59 / 3 / 0 / 0 / 0**; interactions and invariants **25 / 2 / 0 / 0 / 0**, X14 closed as
superseded. **What this does not certify:** Delayed Auction, or any unrelated backlog item — S10-11, S10-27, U-41, U-43,
OD-UR-9's seed source (UR-F16), UR-F15 and the four "needs a harness" rows stay open where they are routed. See
**"UR-8 closure (rev 12)"** and the certification document §Q.

**Verdict (rev 12, UR-8): CERTIFIED at `RULES_ENGINE_VERSION` 10** — Unpredictable Revenue, standalone and with Gentle
Rust; subject to the owner's full-suite gate and commit. *(The letter below is kept as the record of rev 7 – rev 11.)*
...
| 12 | 2026-09-25 | **UR-7 recorded COMPLETE (owner-gated; committed and pushed as `ff7a04b`). UR-8 — the v10 closure (uncommitted, awaiting the owner's full-suite gate).** `RULES_ENGINE_VERSION` **9 → 10**, supported `[10]`, changelog row 10 naming exactly §15's eight replay semantics (OD-UR-1, OD-GR-3, OD-UR-2, OD-UR-3, OD-UR-7, OD-UR-5, OD-UR-10, OD-UR-13) with OD-UR-4 / -6 / -8 / -9 / -11 / -12 and the evidence behind a NOT RULES marker; new closure suite `unpredictableRevenueClosure` (19 tests; owns the current literal) and five suites narrowed, version-literal only (`gentleRustClosure`, the certification game, `carcosaBloodPrice` J1, `revenueTieRounding`, `unpredictableRevenueStats`); a v9 room held, never reinterpreted; the constructed game and the 18-file corpus identical across the boundary but for the pin; **S10-21 / X14 CLOSED — SUPERSEDED**; every owner decision re-verified with durable evidence (certification document §Q.5). Counts 59 / 3 / 0 / 0 / 0 and 25 / 2 / 0 / 0 / 0 (X14 superseded). **Verdict: CERTIFIED at v10** (the variant; not every backlog item — S10-11, S10-27, U-41, U-43, OD-UR-9 stay routed). |
...
moved, and none was re-opened.)* *(Rev 9: **OD-UR-6 implemented** — UR-5; no ruling moved, and none was re-opened.)* *(Rev 10: **OD-UR-8 and OD-UR-12
implemented, and OD-UR-2's boundary notice** — UR-6; no ruling moved, and none was re-opened.)*
...
| **OD-UR-8** | **8-B — the Easter egg stays.** The Rules Reference discloses what a player needs for informed decisions — the Unpredictable Revenue die and rounding already meant to be known, a gilded train's train-limit treatment, its eventual disappearance, and the Blood Price's consequences once OD-UR-5 is decided *(rev 7: it is — the OD-UR-5 row)* — but **not** the Yellow Sign's hidden trigger conditions or odds merely for completeness. | UR-N60, X15; UR-F13 | UR-6, after OD-UR-5 *(rev 7: decided)* *(rev 10: **implemented — UR-6**, UR-F13 fixed; the tie sentence waits for UR-7, UR-F20)* |
...
| **OD-UR-12** | **12-A.** The canonical player-facing name is **Unpredictable Revenue**; later "Unpredictable Routes" copy is normalized to it. Copy only. | UR-N59; UR-F14 | UR-6 *(rev 10: **implemented** — UR-F14 fixed)* |
...
## UR-4 implementation (rev 8, 2026-09-25 — COMPLETE: owner-gated, committed and pushed as `a6183b5` *(rev 9; at rev 8 uncommitted, awaiting the owner's full repository Jest gate)*)
...
* **UR-5 — Carcosan Railways under OD-UR-5(b).** Its blurb ("Saw the Yellow Sign and never paid the Blood Price to be rid
  ...
  (buying) president, exactly as before UR-4 (backlog S9-7, UR-4 block). *(Rev 9 — answered in UR-5: the logic is
  correct — #1421's buyer pays and is The Redeemer, and a released seller never paid, so its president keeps the accolade
  (pinned); only the blurb is stale — UR-6, Appendix B item 13.)*
...
## UR-5 implementation (rev 9, 2026-09-25 — COMPLETE: owner-gated, committed and pushed as `0d8157e` *(rev 10; at rev 9 uncommitted, awaiting the owner's full repository Jest gate)*)

**Scope, as the owner's UR-5 brief set it:** OD-UR-6 (UR-F8) — corporation / turn statistics on the revenue actually
paid (6.1), train / route statistics on the printed value of each completed route (6.1), a Mark-nullified route not
earned (6.2), the Carcosa gift not a purchase and the Blood Price a genuine purchase by the buyer (6.3) — with the
roadmap's two small items, The Cowboy's printed line (UR-F9) and `MINTS_BY_DESIGN`'s stale comment (Appendix B item 11,
OD-UR-4), and a review of Carcosan Railways under OD-UR-5(b). **Not in scope and not touched:** the reducer and every
board, message and digest; UR-6's copy / UI / Rules Reference; OD-UR-10's tie rule (UR-F20, UR-7); the constructed game
(UR-7); the version bump (UR-8). `RULES_ENGINE_VERSION` stays **9**. Starting point: local `main` = `a6183b5` ("ur4+test"
— UR-4, owner-gated), equal to `origin/main`; the tracked tree clean (only the untracked `.claude/`). Files:
`utils/gameHistory.ts` (the one production file); `utils/unpredictableRevenueStats.test.ts` (new);
`utils/yellowSignRunBoundStats.test.ts` (two pins moved to the ruled basis, with a UR-5 note);
`utils/moneyConservation.test.ts` (comments only).

### The statistics pipeline, mapped

Nothing is recorded during play (#1411 / #1414): the history replays the log through the same engine every client uses
and diffs the board before and after each entry.

| layer | what | where |
|---|---|---|
| reducer-authoritative board | `last_route_revenue` (the turn's paid figure — what `DeclareDividends` must declare, audit C1), `printed_route_revenue` (the printed turn total), `last_run_breakdown` (each route's slot, model and printed figure), `last_run_yellow_sign` (the Mark / gift a pinned run applied), `owned_trains` / `carcosan_trains` (the gilding) / `ghost_trains` (provenance), treasuries, cash | `gameEngine/sandboxSession.ts` (run arm, `settleRunYellowSign`, `applyYellowSignOutcome`) |
| reconstructed from history | every tally: the log replayed (`RoomEngine` + `LegacyLogAdapters`), each entry's before / after diffed | `utils/gameHistory.ts` (`gameHistoryFrom`) |
| cached / derived | the round samples (`RoundSample`: each corporation's revenue in the round, written as its run lands, #1420), the tallies, the accolades (`utils/accolades.ts` catalogue + `selectCeremony`), the autopsy (lifetime revenue, dividends, withheld, payback) and the fleet ledger | `gameHistory.ts`, computed once at game end (`App.tsx`, #1411) |
| presentation only | the epilogue charts, the autopsy table and its ledger, the ceremony | `components/EpilogueCharts.tsx`, `AccoladesCeremony.tsx`, `GameOverModal.tsx`, `RoundScrubber.tsx` |

**Where Unpredictable Revenue leaked (UR-F8, UR-F9):** the run's tallies read `runBeforeSign` — the run as priced,
printed, with a Mark-nullified route put back — for lifetime revenue, the Juggernaut, the Revenue-per-OR sample, the
ledger's earned / runs and Master of the Line; the purchase tallies (Fleet Admiral, the ledger's paid split, the Early
Adopter) read every train that joined a roster, the Carcosa gift included; the Cowboy classified the natural flavour line,
rolled even for a run that earned $0.

### Each statistic, by what it measures

| statistic | measures | basis (rev 9) |
|---|---|---|
| lifetime revenue (the autopsy's "Revenue"), The Workhorse | corporation revenue over the game | **paid** — Σ of what each run left for the Dividends step |
| The Juggernaut | the corporation's biggest turn | **paid** |
| the Revenue-per-OR chart (`RoundSample.revenue`) | the corporation's revenue in that round | **paid** (the sample's own fallback order too) |
| White Elephant, Little Engine, the payback | revenue against train spend | **paid** (through lifetime revenue) |
| Robber Baron, Dividend Machine, Golden Goose, The Hobo, dividends paid / withheld, Scrooge Company | money moved at `DeclareDividends` | unchanged — already the money paid |
| Master of the Line | the single most valuable train run | **printed, completed route** |
| the fleet ledger's "earned" and "runs" | a model's trains' completed routes | **printed, completed routes** — a Mark-nullified route contributes nothing, not even a run |
| the fleet ledger's "count" (n) | "the number of trains the corporation held" (#1431's ruling) | holdings — the gift was held, so it is counted, at $0 |
| the fleet ledger's "paid" | the price paid | purchases only — the gift cost nothing |
| Fleet Admiral | trains bought | **purchases** — never the gift |
| The Early Adopter | "the first corporation to buy a Diesel" | **purchases** — never a gifted D; a Blood Price D counts |
| Salt / Sugar Daddy, train spend | intercorporate purchases against the Depot price; money spent | unchanged — the Blood Price is the buyer's intercorporate purchase |
| Phase Rusher, the Gravedigger's cause | the purchase that moved the phase | unchanged — no gift or cured train moves the phase (OD-UR-3, OD-UR-5(a)) |
| The Salvager | trade-ins | unchanged (a pre-existing Bank Pool miscount is recorded, backlog U-43) |
| The Cowboy | runs whose printed flavour line was an animal's | **the printed line** (UR-F9) |
| Carcosan Railways, The Redeemer | the Sign's bearers; the Blood Price's payer | unchanged — below |

### The design

* **A run is booked as it SETTLED, at two levels** (`RunBooking`, `gameHistory.ts`). At each `RunMultipleRoutes` the
  history reads the board the run settled: `last_route_revenue` for the corporation / turn, and `last_run_breakdown` —
  each completed route at its printed figure — for the trains. On a pinned table the Mark lands in that same entry
  (UR-3), so the nullified route is already out of the breakdown and the kept run re-rolled: it is never booked, not booked
  and then subtracted. Where no die is rolled `last_route_revenue` IS the printed total, so a standard game books exactly
  what it always did. The tallies (lifetime revenue, the Juggernaut, the ledger's earned / runs, Master of the Line, the
  Cowboy) are drawn from the bookings once, after the replay, in log order with the old strict "must beat" rule.
* **The legacy request (unpinned boards)** is a later entry of the same turn, so its Mark AMENDS the turn's booking before
  any tally is drawn: the taken train's route leaves the completed routes (`runWithoutTrain(.., null)` — the reducer's own
  remainder, found by slot then model; the other trains keep theirs, including under #1046's seedless "no revenue for this
  submission" zeroing, which took the corporation's revenue, not their routes), and the paid figure becomes the board's —
  unless the Dividends step has already paid the run out (a request delayed past it, UR-F2's shape, changes
  `last_route_revenue`, not the money paid). Only the corporation's ACCEPTED run of THIS turn (the round, the corporation,
  `routes_run_this_turn`) is ever amended.
* **Acquired is not bought.** The purchase tallies stay a roster diff (every way a train joins a roster except one is a
  purchase), minus the gift: **the one copy that arrives gilded**. `applyYellowSignOutcome` writes the train, its
  provenance and its gilding together (#1089), and no purchase delivers a gilded copy (the Blood Price burns the gilding as
  it sells; an ordinary copy's sale leaves it where it was, UR-4) — so a copy is a gift exactly when the entry gilded it at
  the corporation that received it, on either road (the run's entry, or a legacy `YellowSignEvent`). Not provenance: a
  cured copy keeps `ghost_trains` through the Blood Price and a Bank Pool purchase, and both are purchases. A gift opens no
  Fleet Admiral entry at all (a zero would still carry the fleet-value tie-break).
* **The Cowboy (UR-F9)** asks the resolution the shell prints: `narrateRunYellowSign` on a pinned table, the shell's own
  `resolveFlavourLine` call on the replayed flags on an unpinned one; the roll is the turn's as run (the kept total plus a
  nullified route); a run that earned nothing prints no line (#1017), so it meets no animal; a legacy stage that lands
  replaces the line.

### Acquisition paths, classified (Task 9)

| path | purchase? | evidence |
|---|---|---|
| Depot purchase (`BuyHardwareFromPool`, forced `EmergencyBuyHardware`) | yes | L (a Depot D: Early Adopter, ledger $1,100); J / K (NYC's real D after a gifted D) |
| Bank Pool purchase (`BuyHardwareFromPool` naming a returned model) — the cured additional copy included | yes | I (the cured 6, provenance and all: ledger count and price, a Fleet Admiral purchase) |
| intercorporate purchase (`BuyTrainFromCorporation`, the offer's derived settlement included) | yes — the base game's reading, unchanged | L (the buyer's Fleet Admiral and ledger) |
| the Blood Price | yes — the BUYER's; the seller is credited with no purchase (its copy is `sold`) | H (a cured 6), K (a cured D: the buyer is the Early Adopter), the Carcosan Railways case |
| the Diesel exchange | yes — unchanged | I, L (Early Adopter; Salvager) |
| the Carcosa gift | **no** | G (a 6), J (a D), M (hosted) |
| the Mark, the fog, rust, a discard, a Gentle Rust Final Run | no — they only remove trains | C, C2, C3; UR-3's `yellowSignRunBoundStats` |

### Carcosan Railways under OD-UR-5(b) — the finding

The qualification logic is **correct** and unchanged: #1421 defines the accolade as "a president who lived through any of
the sequence — the Mark, the gift, the fog — without paying the Blood Price", and The Redeemer as the one who "paid the
Blood Price and took the Carcosan train off another corporation" — the BUYER, which the history has always credited (and
UR-4 made the reducer agree: the buyer's marker moves). So a seller released by a buyer's Blood Price saw the Sign and
never paid it: its president keeps Carcosan Railways, and the buyer's president is The Redeemer (pinned:
`unpredictableRevenueStats` "Carcosan Railways and The Redeemer"). **Only the wording is stale** — the blurb "never paid the
Blood Price to be rid of it" reads as though the Carcosan corporation could pay to be rid of the train, which under
OD-UR-5(b) it cannot: copy, UR-6 (Appendix B item 13).

### Finding status (rev 9)

| id | status | evidence (durable, `frontend/src/utils/`) |
|---|---|---|
| UR-F8 | **FIXED** (every table; the pinned run and the unpinned legacy request) | `unpredictableRevenueStats.test.ts` A, B, C, C2, C3, G, J / K, H / K, I, M, the JUNO-Z6C-494 case; `yellowSignRunBoundStats.test.ts` (the two pins moved to the ruled basis) |
| UR-F9 | **FIXED** (every table) | `unpredictableRevenueStats.test.ts` UR-F9 (the gift replacing an animal line, pinned and unpinned; a $0 run; the control) |
| UR-F10 | closed (rev 2); **its stale comment corrected** (Appendix B item 11) | `moneyConservation.test.ts` (comments only; its assertions and the case's title unchanged — the Batch 7.3 / 7.5 records cite the title) |

**Reproduced before fixing.** The new suite (46 tests) run against the untouched `a6183b5`: **22 fail, 24 pass** — the
24 are the premises (the boards the reducer builds) and the controls that must hold on both trees (the Blood Price and
Bank Pool purchases, the Diesel exchange, standard play, determinism). The two moved `yellowSignRunBoundStats` pins fail on
the baseline, as they must. All pass on the tree.

### Corpus — boards unchanged; derived statistics move only on Unpredictable Revenue logs

The 18-file reconciliation (baseline vs tree): 4,105 stored / 3,731 applied / 374 dropped / 3,763 engine applications; 0
state, grid, cursor, derived-action and known-phase differences; 18 / 18 final boards equal; the known phase never
decreases. Y8V `b4fae877c35604fe`; 3XD `74db6e4bad736fec`. **The derived statistics** (every accolade, the ceremony, the
autopsy with its ledgers, every round sample) were dumped for all 18 files on both trees: **identical on 14** — every
file that does not play the variant (golden 7NZ / CV4 / G6J; server 7NZ, CV4, G6J, TQQ; export CV4, JJD, QVC, and Y8V,
which carries no deal) and three variant files
whose runs moved no figure (server 8E8, export 3XD, the FCJ-96 prefix) — and changed only on CW7, FCJ, Z6C and Z6C-494,
all Unpredictable Revenue, in
the ruled directions: corporation revenue to the paid figure (The Workhorse, The Juggernaut, lifetime revenue, the payback,
the Revenue-per-OR chart, and with them White Elephant's runner-up and Little Engine's tie fraction), and The Cowboy's two
phantom run-ins removed (FCJ 167 and Z6C 490: runs the current authority refuses, $0, no line printed). No per-train
figure moved (no corpus run was Marked on the replay) and no purchase figure moved (the corpus holds no gift). No corpus
file, fixture, golden or log was touched.

### Deferred and recorded (not implemented here)

* **Backlog Part C U-43 (new) — pre-existing statistics residuals, standard games too:** The Salvager counts a Bank Pool
  purchase (`BuyHardwareFromPool` naming `returned_model_type`, #1314) as a trade-in; a refused `RunMultipleRoutes` is
  booked at the figures the board still holds (harmless $0 at a turn's start in every corpus case, but a refused duplicate
  mid-turn would book twice); legacy `RunManualRoute` runs are not booked at all (JUNO-Y8V's 92); a pre-#1051 seedless UR
  run is skipped by The Cowboy though the shell printed its line. None is UR-specific; each would change standard
  statistics; each needs its own review.
* **UR-6 — copy:** Carcosan Railways' blurb (Appendix B item 13); under the variant the Revenue-per-OR chart's hint ("what
  each corporation's trains ran for") and the ledger's "Runs" column describe the paid figure and the completed runs
  (Appendix B item 14).
* **Judgment call, for the owner's eye (no owner decision is needed to ship it):** Fleet Admiral breaks a tie of purchase
  counts on the value of the final fleet (#1416), which includes a gift the corporation still holds — a holding, not a
  purchase; left as it is.
* **The Cowboy's legacy corners (unpinned boards only):** the replayed flags stand in for the shell's log-text fallback,
  and a playtest waiver is seen only where its request lands.
...
authority, the train-sale schema, UR-5's statistics semantics, OD-UR-10's tie rule (UR-F20 — UR-7), the constructed
certification game (UR-7), the version bump (UR-8). `RULES_ENGINE_VERSION` stays **9**. Starting point: local `main` =
`0d8157ecc38edeff4da4729b5c5681b0103b4aaa` ("Implement Unpredictable Revenue statistics" — UR-5, owner-gated), equal to
`origin/main`; the tracked tree clean (only the untracked `.claude/`); no `.git/index.lock`.

### The durable obligations found (before editing)

1. **UR-F13 / OD-UR-8 (D-42)** — the Rules Reference's disclosure: the die and its rounding, the per-turn aggregate, the
   gilded train's limit treatment and disappearance, the Blood Price's consequences (OD-UR-5, fully decided; UR-F22's "the
   copy with UR-6"); the Yellow Sign's trigger conditions and odds hidden.
2. **UR-F14 / OD-UR-12 (D-45)** — "Unpredictable Routes" on the host setup and the room list.
3. **UR-F12** — the gilded chip's tooltip ("until this Operating Round ends").
4. **U-42's copy** — the Activity Log's offer and trade lines named the model, not the copy.
5. **The independent UR-4 review** (2026-09-25; recorded here for the first time): **D1** — the consent prompt and the
   pending-offer view showed the Blood Price only for `offer.gilded === true`, though an UNNAMED offer is settled as the
   Blood Price when the seller holds only the gold-trimmed copy of the model (with its missing test **T5**); **D5** — a
   stale "OD-UR-5 is OPEN" comment in `carcosaFogBoundary.test.ts`.
6. **Appendix B** items 1 – 10 (stale comments), 12 (= UR-F12), 13 (Carcosan Railways' blurb), 14 (the Revenue chart's
   hint and the ledger's "Runs" — optional).
7. **UR-3's deferrals routed to UR-6** ("UR-3 implementation (rev 3)"; backlog UR-3 block): the fog's formal boundary
   notice (OD-UR-2 / D-38); a greyed gilded chip in the Diesel trade-in row, in GR-3's U-11 manner; a player-facing line
   that the Mark's train is gone for good.
8. **The brief's own check:** The Redeemer's copy.
9. **A dependency, not UR-6 work:** the exact-$5 tie (OD-UR-10 = 10-C, UR-F20) is UR-7's, so no copy may claim either tie
   rule yet.

None conflicted with the brief. Item 7's notice was taken as the fog's RULED cue (#1092's sound, #1093's film) at the
boundary where OD-UR-2 moved the fog; no modal was ruled, so none was invented.

### What changed, surface by surface

* **D1 — the consent prompt and the pending-offer view.** `utils/saleCopyDisclosure.ts` (new, presentation only):
  `saleCopyKind(state, seller, model, gilded)` answers `bloodPrice` / `ordinaryBesideGilded` / `plain` by asking the
  authority's own predicates — `isCarcosanTransfer` (the reducer's, which the chart step asks on the board before the
  sale) and `resolveSaleCopy` / `saleCopies` (the sale authority's multiset reading); `offerSettlesAsBloodPrice(state,
  offer)` is the first of those for a standing offer. `App.tsx`'s `sandboxTrainProposal` derives
  `TrainTradeProposal.bloodPrice` (display only, never sent; `[gameState]` is the memo's dependency — the answer reads the
  seller's fleet) and `TrainTradePrompt` reads it, not the wire field: the Blood Price line and "the gold-trimmed" now
  show for a named gilded offer AND for an unnamed offer for the seller's only gold-trimmed copy, never for an ordinary
  copy. The same prompt is the pending-offer view every other seat reads ("Waiting on …"). An ambiguous unnamed offer is
  refused at the proposal by the authority, so no wording is needed for it; were one read, it would claim no Blood Price.
  The contract path (`liveTrainOffer`) has no Carcosa and is unchanged.
* **U-42 — the Activity Log names the copy** (`utils/actionLog.ts`, asked of the BEFORE board — the one the proposal,
  answer and settlement are judged on):
  * the gold-trimmed copy: "C&O offers $300 for B&O's gold-trimmed 6-train — buying it is the Blood Price. p2 must
    answer." / "p2 accepted $300 for B&O's gold-trimmed 6-train." / "C&O bought B&O's gold-trimmed 6-train for $300,
    paying the Blood Price: it is an ordinary 6-train now, and B&O is released from the Carcosan curse." (+ treasuries);
  * an ordinary copy beside a gold-trimmed one: "… for an ordinary 6-train from B&O — not the gold-trimmed one, so no Blood
    Price." / "… for B&O's ordinary 6-train." / "C&O bought an ordinary 6-train from B&O for $300 — not the gold-trimmed
    one, so no Blood Price.";
  * every other sale (every standard game; a seller with no gold-trimmed copy of the model): the sentences before UR-6,
    byte for byte.
  The buyer-only market consequence keeps its own line — #1090's ruled sentence, "A Blood Price was paid: [ticker]'s stock
  dropped from $X to $Y", whose ticker is the token the chart step moved: the BUYER's since UR-4 (pinned: the report's
  `companyId` is the buyer's, the seller's position unchanged). No line says the seller paid or moved, or leaves the buyer
  a gold-trimmed train. The shell's local same-president / online line uses the same `saleCopyKind`.
* **UR-F13 — the Rules Reference** (`components/RulesReference.tsx`). The one-sentence Dividends note is replaced by two
  blocks tagged "Unpredictable revenue", in GR-3's shape (#1702: `OpBlock`, the page's measure, lists; no disclosure):
  * **Dividends — "Unpredictable Revenue"** (`UNPREDICTABLE_REVENUE_RULES`): one die per turn on the total of all the
    corporation's routes; the face table (1 → 80%, 2 → 90%, 3 or 4 → 100%, 5 → 110%, 6 → 120% — read against
    `REVENUE_MODIFIER_BY_FACE` in the test), in a scroll region; "rounded to the nearest $10", with a non-tie example ($80
    at 80% → $64 → $60) and "a run worth anything still pays at least $10"; route choice and the highest revenue rule on
    printed values, the die only when the run is made; the paid figure is what is paid out or withheld and what moves the
    share price; Private Company income never rolled; an undo does not roll again. **The Yellow Sign — consequences only:**
    "What calls it is not revealed"; a taken train is the lowest-value one, its route earns nothing that turn and the rest
    of the run is paid at the same roll, it is **removed from the game for good** — not the Bank, not the Bank Pool, and the
    phase never goes back (OD-UR-13; UR-3's "gone for good" deferral) — and the corporation receives half its face value
    (`markPayout`); a trainless corporation meets the ordinary rules; and, only where Gentle Rust is also on, a train
    awaiting or on its Final Run is never the one taken (OD-GR-3).
  * **Buy Trains — "Gold-trimmed Carcosa trains"** (`CARCOSA_TRAIN_RULES`): an extra train, not one taken from the Bank;
    while gold-trimmed it takes no limit slot but is owned (not trainless), never changes the phase (OD-UR-3), is no
    Diesel trade-in while an ordinary copy is (OD-UR-7), and carries the Carcosan curse, which the fog does not lift; **the
    fog** — the end of the Operating Round set after the one in which a Diesel is first bought from the Bank (or in which
    the train arrived, if a Diesel came first), whether or not its corporation operates, for nothing; a gold-trimmed Diesel
    is not a Diesel bought from the Bank (OD-UR-2, UR-N43); **the Blood Price** — the ordinary intercorporate rules; the
    BUYER pays the price and its share price moves 1 left and 1 down, the seller's does not move and it is released
    (OD-UR-5(b)); the buyer receives an ordinary train — its limit, running, rust, an ordinary resale, a Diesel trade-in, no
    fog (5a-1); no phase change, "like every purchase from another corporation" (OD-UR-5(a), rev 6); the offer names the copy
    and the ordinary copy's sale is an ordinary sale (5c-2).
  Nothing names a phase window, a die face that calls the Sign, a chance, or who the Sign can visit next (pinned: the
  blocks say none of it, and the variant adds none of it anywhere in the reference — every page counted with and without
  it). No field names, provenance or replay vocabulary. **No tie direction:** the engine still rounds an exact $5 tie up;
  UR-7 owes the tie sentence here when it lands 10-C (the block's note says so).
* **UR-F12 — the chip.** `CARCOSA_CHIP_TOOLTIP` (`components/TrainBadges.tsx`): "Gold-trimmed Carcosa train — a gift of the
  Yellow Sign. Occupies no train-limit slot while it stays gold-trimmed, and cannot be traded in for a Diesel. It vanishes
  into the fog at the end of the Operating Round set after the one in which a Diesel is first bought from the Bank (or after
  the set it arrived in, if a Diesel came first) — unless another corporation buys it first and pays the Blood Price." The
  chip has no board, so it states the rule, anchored to the moment the deadline counts from. Its `alt` reads "Gold-trimmed
  Carcosa train" (was "Carcosa ghost train"), the panel's name for it. A cured train wears no sign.
* **UR-F14 — the name.** `HostSetupCard.tsx` and `LobbyRoomList.tsx` say "Unpredictable Revenue". The sentence-case label
  lists (`VARIANT_COPY` "Unpredictable revenue", beside "Gentle rust (easier)" and "Dynamic stock market"; the Rules
  Reference's tag, beside "Gentle rust") keep their lists' case — the same name; the headings are Title Case, as Gentle
  Rust's are.
* **Appendix B item 13 — Carcosan Railways** (`utils/accolades.ts`): "Saw the Yellow Sign touch their railroad, and never
  paid a Blood Price to buy a gold-trimmed train." True of a released seller (it bore the Sign; the buyer paid). The
  qualification is unchanged (UR-5 pinned it). **The Redeemer:** "Paid the Blood Price and took the Carcosan train off
  another corporation." — the buyer's act, credited to the buying president (`gameHistory.ts`); unchanged.
* **Appendix B item 14:** the Revenue chart's hint is kept — "ran for" is the Activity Log's own word for the paid figure;
  the ledger's titles now say what the columns count on every table ("The printed value of its completed routes";
  "Completed runs, counting each train separately").
* **UR-3's deferral — the Diesel trade-in row.** `gildedExchangeCopies` / `gildedExchangeCopyReason`
  (`gameEngine/dieselExchange.ts`, presentation helpers beside GR-3's Final Run pair; the refusal is still
  `dieselExchangeRefusal`, unchanged) and `DieselExchangeOffer.gilded` (present only where the buyer holds such a copy,
  so every standard offer keeps its shape key for key). The panel draws the gold-trimmed copy greyed and struck through,
  with the reason as title, accessible name and a line under the row; a sole gold-trimmed copy keeps the row, greyed,
  beside the gate's own sentence. Exchangeable, Final Run and gold-trimmed copies partition the eligible trains.
* **UR-3's deferral — the fog's formal notice** (`App.tsx`, the set-end branch): after the Activity Log line (UR-3's,
  unchanged), the fog's ruled cue — `variantCueFor({ stage: "fog" })`: `carcosan-train.mp3` through `playVariantCue`
  under the same toggles as the run's cues, and `carcosan-train.mp4` through the haunting overlay — once per boundary,
  guarded by `replayingHistory` (#1094: a refresh or an Undo does not raise it again). No modal.
* **Appendix B items 1 – 10 and D5** — bracketed "[UR-6 …]" annotations beside the stale text (kept as the record) in
  `gameVariants.ts` (1, 2), `gameState.ts` (3), `sandboxSession.ts` (4, 5, 6), `trainDiscard.ts` (7), `yellowSign.ts` (8),
  `App.tsx` (9, 10); `carcosaFogBoundary.test.ts` (D5); and the "to be rid of it" notes in `gameHistory.ts` /
  `unpredictableRevenueStats.test.ts`.

### Finding status (rev 10)

| id | status | evidence (durable, `frontend/src/`) |
|---|---|---|
| UR-F12 | **FIXED** | `components/unpredictableRevenueDisclosure.test.tsx` (UR-F12); `utils/batch59.test.ts` (pins moved, UR-6 note) |
| UR-F13 | **FIXED** (the tie sentence waits for UR-7 — UR-F20) | `components/unpredictableRevenueRulesReference.test.tsx` (15) |
| UR-F14 | **FIXED** | `unpredictableRevenueDisclosure` (UR-F14: the two rows; no "Unpredictable Routes" in the Lobby sources) |
| D1 (independent UR-4 review) | **FIXED** | `unpredictableRevenueDisclosure` D1 1 – 7 (T5 is case 2: an unnamed offer for the only gold-trimmed copy — disclosed, then settled as the Blood Price by the reducer); `components/carcosaBloodPricePanel.test.tsx` (its proposal now carries the shell's derivation) |
| D5 (independent UR-4 review) | **FIXED** (comment) | `utils/carcosaFogBoundary.test.ts` |
| U-42 (copy) | **RESOLVED** | `unpredictableRevenueDisclosure` U-42 |
| Appendix B 1 – 14 | **RESOLVED** (11 in UR-5) | the annotations; `unpredictableRevenueDisclosure` (13, 14) |
| UR-3's deferrals | **RESOLVED** | `unpredictableRevenueDisclosure` (the trade-in row; the fog's cue — source pins, no App harness); `utils/batch60.test.ts` (UR-6 notes); the Rules Reference ("removed from the game for good") |

**Reproduced before fixing.** The two new suites (40 tests) were run against the untouched tracked tree of `0d8157e`
(the new helper present so the suites compile): **25 fail, 15 pass** — the 15 are the controls and the premises that hold
on both trees (a named gilded offer and an ordinary one already disclosed correctly since UR-4; the standard game; the
refused ambiguous offer; the buyer-only chart report; the plain sentences; the cured chip; the standard trade-in offer; the
fog's cue constants; The Redeemer; and five Rules Reference checks that hold vacuously there, the blocks not existing —
the standard-mode ones, the stale-language and no-tie scans, and the whole-page counts). All 40 pass on the tree.

### Replay neutrality — confirmed

The engine files UR-6 touched — `sandboxSession.ts`, `gameState.ts`, `gameVariants.ts`, `yellowSign.ts`,
`trainDiscard.ts` and `utils/gameHistory.ts` — are **comment-only**: each, transpiled with comments removed, is
byte-identical to `0d8157e`'s. The only runtime change under `gameEngine/` is presentation: two new helpers in
`dieselExchange.ts` and an optional key on the Buy Trains panel's offer, none called by the reducer, the server or replay.
No message, schema, state field, digest, log, fixture, golden or corpus file changed, so no corpus reconciliation was
needed and none was run; nothing was repinned.

### Validation

* **Focused Jest.** On a clean clone of `0d8157e` with UR-6's 24 files laid over it (the cloud workspace; every test file
  that names a touched module or an Unpredictable Revenue term): **335 suites / 6,290 tests — 328 suites pass**; the other
  **7** (`replayJuno3XD`, `replayJunoCV4`, `stage10Closure`, `gentleRustClosure`, `trainDiscard`, `moneyConservation`,
  `stage103bChartCoreAtomicity`) read the gitignored local corpus and fail identically on the untouched baseline there —
  **all 7 pass on the owner's checkout** (125 tests), where the corpus lives. On the owner's checkout the directly affected
  suites were also run (the two new ones, `carcosaBloodPricePanel`, `carcosaBloodPrice`, `carcosaBloodPriceStats`,
  `carcosaFogBoundary`, `carcosaGildedExchange`, `unpredictableRevenueStats`, `unpredictableRevenueStandardControls`,
  `yellowSignRunBoundStats`, `accolades`, `gameHistory`, the five Rules Reference suites, `gentleRustPresentation` (both),
  `dieselExchange`, `trainOffer`, `logCondense`, `lobbyPublicRooms`, `variantWiring`, `variantCopy`, `batch46`, `batch59`,
  `batch60`, `batch62`, `flourishAudio`, `stage95GhostLimit`): all green. Full Jest was not run (the owner's gate).
* **tsc** (frontend, server; TypeScript 4.9.5, the owner's checkout): 0.
* **Build** (`react-app-rewired build`, the cloud clone — a build on the owner's checkout cannot finish inside this
  session's 180-second command window, and cannot clear the connected `build/` folder): exit 0; **49** ESLint warning
  lines, identical (modulo line numbers) to the untouched baseline built the same way; the workspace's own source-map
  notices and the "Critical dependency" notice likewise identical.
* **Mobile / layout:** both blocks are `OpBlock`s at the page's 68ch measure with `minWidth: 0`; the one table sits in the
  page's own `overflow-x: auto` region; no control, disclosure or fixed-width table (pinned). The panel's greyed chips use
  the Final Run chip's style in the same wrapping row.
* **Independent review** (a reviewer agent with no part in the work; read-only): no blocker. Its two SHOULD FIX items —
  "half the taken train's price" (the award is half the face value, whatever the train was bought for) and the chip's
  "next Operating Round set" (a deadline stated against an unstated moment) — were applied, with pins; its nits are below.
...
## UR-7 implementation (rev 11, 2026-09-25 — COMPLETE: owner-gated, committed and pushed as `ff7a04b` *(rev 12; at rev 11 uncommitted, awaiting the owner's full repository Jest gate)*)
...
* **UR-8** — the 9 → 10 boundary and the closure record (§15). Until then Unpredictable Revenue is not certified.
  *(Rev 12: done — "UR-8 closure (rev 12)" below.)*

---

## UR-8 closure (rev 12, 2026-09-25 — uncommitted, awaiting the owner's full-suite gate)

**Scope, as the owner's UR-8 brief set it:** the deliberate `RULES_ENGINE_VERSION` 9 → 10 boundary; the replay-semantic
record; final certification closure; S10-21's disposition; a check that no Unpredictable Revenue blocker remains. Not a
gameplay pass: no certified mechanic was re-opened, and no production code changed except the version constant and its
changelog row. **The evidence is the certification document's §Q**; this section is its summary here.

**Starting point.** `main`; local HEAD = `origin/main` = `ff7a04b6aaea17597cf9c5ea0bd61bcc8b0632e8` (UR-7; UR-6 `cd889cb`
pushed with it) — 0 ahead / 0 behind; the tracked tree clean (only the untracked `.claude/`); no `.git/index.lock`. Worked
in a cloud clone of `origin/main` with the 13 git-ignored corpus files copied from the owner's checkout (md5 verified);
only the changed files were written back.

### The durable obligations found (before editing)

1. **§15, the UR-8 row** — the one 9 → 10 boundary; changelog row 10 naming exactly the UR replay semantics; closure
   tests (supported `[10]`, a v9 log refused); the corpus and G1 / G0 across the boundary; backlog S9-7 / Part D / Part E;
   the owner's full-suite gate.
2. **§15, "The version boundary"** — the eight items (1) – (8) and the list named NOT rules (OD-UR-4, OD-UR-6, OD-UR-8,
   OD-UR-12, OD-UR-11, the evidence, OD-UR-9).
3. **UR-7's routing** — S10-21 / X14 "UR-8 or a named follow-up — the planner's call"; §6.4 "re-verified across the
   closure boundary in UR-8".
4. **The certification document §P** — the same list, plus "the S10-21 decision".
5. **The GR-5 precedent** (`gentleRustClosure.test.ts`, the Gentle Rust certification document §P) — a closure suite owns
   the current literal; the previous owner is narrowed, version-literal only; Part E gains the row.
6. **The version-literal pins UR-4, UR-5 and UR-7 left** ("still 9 — owed to the 9 → 10 boundary"): `carcosaBloodPrice`
   J1, `unpredictableRevenueStats`, `revenueTieRounding`, the certification game — each written to change at this bump.

None conflicted with the brief. One reading recorded: the brief lists "statistics corrections" among the accumulated
changes "as applicable"; by #1520's policy and the GR-5 / U-9 precedent they are derived history, so row 10 names them
behind the NOT RULES marker rather than as a replay semantic (§15 says the same).

### What changed

* `frontend/src/gameEngine/rulesVersion.ts` — `RULES_ENGINE_VERSION = 10`; changelog row 10. Nothing else: the
  compatibility machinery, the policies and the presence-only seams are untouched.
* `frontend/src/utils/unpredictableRevenueClosure.test.ts` (**new**, 19 tests) — the bump; row 10's eight numbered
  semantics and its NOT RULES marker (U-41, U-43, S10-21, S10-27 and Delayed Auction absent); the v10-only matrix with 9
  as the prior version, including a v9 Unpredictable Revenue + Gentle Rust room held before the reducer sees an entry;
  no authority behind (1) – (8) compares or names the version; the constructed game dealt at 10; the 18 corpus files
  unpinned with totals 4,105 / 3,731 / 374.
* Narrowed, version-literal only: `gentleRustClosure` (current `>= 9`, a row prefix, the matrix on the current version —
  row 9 and the v8 cases unchanged), `unpredictableRevenueCertificationGame` (`>= 10`, the pin = the engine's),
  `carcosaBloodPrice` J1 and `revenueTieRounding` (`>= 10`, "carried"), `unpredictableRevenueStats` (`>= 9`, "UR-5 moved
  no version").

### Replay / version policy — the closure question answered

**Is v10 the correct version for all new authoritative games while historical v9 material remains handled by the
established policy? Yes.** The server stamps 10 over any claim; a v10 room restores, rebuilds and replays under
`SERVER_REPLAY_POLICY`; a v9-pinned room — the only historical v9 material there can be, since the development corpus is
unpinned — is `incompatible` and held before a single entry is applied, under the server's policy and the corpus's alike,
and nothing rewrites its pin: it is refused, never reinterpreted, exactly as a v8 room was at GR-5. No version-value
branch exists or was needed (UR-3's seam asks for the pin's presence). No migration, fixture repin or historical-log
rewrite was required. **No blocker.**

### S10-21 — CLOSED — SUPERSEDED

S10-21 (Batch 7.5, category D — test substrate, "not a closure blocker; no rule depends on it") asked for a completed
Unpredictable Revenue game restoring assertions displaced from JUNO-Z6C. The UR-7 constructed certification game is the
stronger evidence for every Unpredictable Revenue assertion it carried — played through the server's own path, pinned,
deterministic, with a standard control, and asserting exactly (Carcosan Railways, The Redeemer, the Gravedigger $1,380
and the Rust Belt $940, the Mark, the gift, the fog, the Blood Price) what S10-21 could only have asserted as non-null.
Its non-UR remainder — the Bagholder / Little Engine detail formats on a completed game and the more-than-ten-OR
timeline — is generic epilogue coverage, recorded on S10-21's closure entry in the backlog and cross-referenced from U-43
(the open statistics / residual pass it belongs to). *(Correction:
UR-7's text says the game restores "a Bagholder detail"; it does not assert one.)* Certification document §Q.4.

### Finding status (rev 12)

| id | status | evidence |
|---|---|---|
| the 9 → 10 boundary (§15, UR-8) | **DONE** — row 10, `[10]` | `utils/unpredictableRevenueClosure.test.ts` |
| X14 / S10-21 | **CLOSED — SUPERSEDED** | certification document §Q.4 |
| UR-F15 (INFO) | open, optional hardening (unchanged) | — |
| UR-F16 (OD-UR-9) | open, routed to AWS / live multiplayer (unchanged) | — |

No DEFECT and no OWNER DECISION row. Every owner decision (OD-UR-1 … OD-UR-13, OD-GR-3) re-verified with durable
evidence — certification document §Q.5; none re-opened.

### Across the boundary

* **Constructed game:** G1 tails A (322 entries, 188 submissions, 20 draws) and B (330, 192, 20) and G0 (65, 45, 4)
  identical on the v9 and v10 trees step by step and entry for entry; final digests differ only by the pin
  (without it: `cb0c2a961d71a467`, `0e7fd01bdc151b92`, `67cda49c2c184313` on both sides); each replays to its final
  board under `SERVER_REPLAY_POLICY`.
* **Corpus (18 files):** 4,105 stored / 3,731 applied / 374 dropped; 3,763 engine applications; state / grid / cursor /
  derived-action / known-phase differences **0 / 0 / 0 / 0 / 0**; final boards equal **18 / 18**; no phase decrease; all
  unpinned; Y8V 668 / 628 / 40, 632 applications, OR 13, **`b4fae877c35604fe`**; 3XD 322 / 320 / 2, 323 applications,
  SR 12, **`74db6e4bad736fec`**; md5 unchanged.

### Validation

Focused Jest in the cloud clone (corpus present): **151 suites / 3,215 tests, all pass**; the closure suite and the three
`>= 10` cases fail against the v9 constant (they measure the boundary). On the owner's checkout after the write-back (blob
hashes verified): 25 suites / 485 tests in five batches — the closure, version, certification-game, Blood Price,
rounding, statistics, Yellow Sign, Carcosa and corpus suites — all pass. `tsc` frontend 0, server 0. Production build (cloud clone): exit 0, 49 warning lines,
identical to the UR-7 tree. `git diff --check` clean. Full Jest NOT run (the owner's gate). Nothing committed or pushed.

### Deferred and recorded (not closed here, routing unchanged)

S10-11 (the unpinned legacy path; AWS / live multiplayer) · S10-27 (the derived $0 withhold; a later derived-action pass)
· U-41 and U-43 (statistics residuals; the dedicated statistics / residual pass) · OD-UR-9 / UR-F16 (hosted seed source;
AWS / live multiplayer) · UR-F15 (INFO) · UR-N9 / X7, UR-N11, UR-N24, R12 (need a harness, not a rule) · S10-21's non-UR
remainder (above) · Delayed Auction (uncertified). None is an Unpredictable Revenue certification blocker.
...
| id | clause | authority | status | conformance |
|---|---|---|---|---|
| UR-N55 | Carcosan Railways (lived through any stage, never redeemed) and The Redeemer (paid the Blood Price). | #1421 | CONFIRMED | conforms by reading; **no live coverage (S10-21)**; *(rev 5: The Redeemer reads the model — under 5c-2 it must read the copy sold, UR-F21)*; *(rev 7: under OD-UR-5(b) the buyer pays the Blood Price, and The Redeemer already credits the buyer's president — no change)* *(rev 8: The Redeemer reads the gilding the sale burned — the copy, not the model; UR-4, `carcosaBloodPriceStats`; Carcosan Railways under the buyer-pays rule → UR-5)* *(rev 9: UR-5 — Carcosan Railways conforms under OD-UR-5(b): #1421's payer is the buyer, so a released seller's president keeps it and the buyer's is The Redeemer, pinned [R] in `unpredictableRevenueStats`; only the blurb is stale → UR-6, Appendix B item 13)* |
...
| UR-N57 | The Cowboy counts the runs whose flavour line was an animal line. | #1429, #1438 | CONFIRMED (catalogue) | ~~**DEFECT UR-F9** (reads the natural line, not the printed one)~~ *(rev 9: **conforms** — UR-5 reads the printed line: the pinned run's applied stage, the unpinned run's legacy resolution, no line for a $0 run; UR-F9 fixed)* |
| UR-N58 | Which figure revenue statistics use under the variant (printed or paid), whether a Mark-nullified run counts as earned, and whether a synthetic gift counts as a purchase. *(Rev 5, OD-UR-6: the revenue actually paid — 6.1, in principle; corporation / turn level; a per-train figure's allocation is an isolated open sub-question — a Mark-nullified route is not earned (6.2); a synthetic gift is not a purchase, a later Blood Price acquisition is (6.3).)* *(Rev 7: the per-train question DECIDED — a per-train statistic uses the printed value of the train's successfully completed route, not an allocation of the paid total; a Mark-nullified route contributes nothing.)* | none; **OD-UR-6 (rev 5)** | ~~**OPEN → OD-UR-6**~~ CONFIRMED *(rev 5; the per-train question decided rev 7 — OD-UR-6 fully decided)* | today: printed; nullified runs counted; gifts counted as bought — **DEFECT UR-F8** (UR-5); *(rev 7)* per train, today's printed basis conforms for completed routes — only the nullified route's inclusion does not *(rev 9: **conforms** — UR-5: corporation / turn on the paid figure, train / route on the printed completed routes, a Mark-nullified route nothing, the gift in no purchase statistic, the Blood Price the buyer's purchase; UR-F8 fixed)* |

### K. Presentation

| id | clause | authority | status | conformance |
|---|---|---|---|---|
| UR-N59 | Lobby / waiting-room copy: "Running railways is risky. In this variant, runs can produce up to ±20% their standard revenue, rounded to the nearest $10." | #961 and its rounding-clause correction | CONFIRMED (copy) | conforms; the variant's TITLE differs by screen (UR-F14 → OD-UR-12; *rev 2:* the name is **Unpredictable Revenue**) *(rev 10: **conforms — UR-F14 FIXED (UR-6)**: the host setup and the room list say "Unpredictable Revenue"; the blurb, which states no tie rule, is unchanged)* |
| UR-N60 | The Rules Reference is the sole player-facing authority. *(Rev 2, OD-UR-8 = 8-B: it discloses what a player needs to act — the die and rounding, a gilded train's train-limit treatment and eventual disappearance, the Blood Price's consequences once OD-UR-5 is decided *(rev 7: it is)* — and keeps the Yellow Sign's trigger conditions and odds hidden.)* | S9-4; **OD-UR-8 (rev 2)** | CONFIRMED | UR has one sentence (`RulesReference.tsx:613–614`) — **DEFECT UR-F13** against 8-B *(rev 10: **conforms — UR-F13 FIXED (UR-6)**: `UNPREDICTABLE_REVENUE_RULES` / `CARCOSA_TRAIN_RULES`, consequences only; the exact-tie sentence waits for UR-7 — `unpredictableRevenueRulesReference`)* |
| UR-N61 | Train chips mark gilded trains and state their limit status truthfully. | #1088, #1089, #1672 | CONFIRMED | **DEFECT UR-F12** (stale tooltip) *(rev 10: **conforms — UR-F12 FIXED (UR-6)**: `CARCOSA_CHIP_TOOLTIP` — the exemption while gold-trimmed, no trade-in, the fog at the end of the set after the doom trigger, the buyer's Blood Price)* |
...
*(Rev 12: **CLOSED — SUPERSEDED** by the UR-7 constructed certification game; its non-UR remainder recorded on the backlog
item — "UR-8 closure (rev 12)".)*
...
*(Rev 9 — IMPLEMENTED, UR-5; UR-F8 and UR-F9 fixed.)* The table above describes the code before UR-5. Now: The Workhorse
and lifetime revenue, The Juggernaut, the Revenue-per-OR chart, White Elephant and Little Engine read the paid figure the
run settled (`last_route_revenue`); Master of the Line and the fleet ledger's earned and runs read the printed completed
routes, a Mark-nullified route contributing nothing — on either road the Mark takes (the pinned run's own entry, or a
legacy request amending the turn's booking); the Carcosa gift is in no purchase statistic (Fleet Admiral, the ledger's
paid, The Early Adopter) while the ledger still holds it at $0 (#1431's n is "trains held" — so the rev 5 line above that
listed "the fleet ledger's count" among the purchase statistics is read against #1431's own definition: the count is a
holding, and the gift is not a purchase in it because it is never booked as one, at $0); the Blood Price is the buyer's
purchase; The Cowboy reads the printed line. The dividend family, Salt / Sugar, Phase Rusher and the rust-cause credit
needed nothing. See "UR-5 implementation (rev 9)" for the classification of every statistic and the acquisition paths.
**One pre-existing, non-UR statistics residual set is recorded** (backlog U-43: the Salvager's Bank Pool miscount, the
booking of a refused run, unbooked legacy `RunManualRoute` runs, the Cowboy's seedless runs), U-41 still untouched.
...
| **UR-F8** | MEDIUM | statistics | Revenue statistics use printed figures; a Mark-nullified run is booked as earned; a synthetic gift counts as a purchase (Fleet Admiral, ledger) and a gifted D can be "the first Diesel" (Early Adopter). | §10 (static) | ~~OD-UR-6 — **OPEN**~~ OD-UR-6 **decided** (rev 5: 6.1 paid, in principle; 6.2; 6.3) — UR-5 · **OPEN — not implemented**; ~~the per-train allocation is 6.1's isolated sub-question~~ *(rev 7: decided — per train, the printed value of the completed route; OD-UR-6 fully decided)* · **FIXED rev 9 (UR-5)** — runs booked as they settled: the paid figure for the corporation / turn, the printed completed routes per train, a Mark-nullified route nothing (pinned and legacy roads); the gift (the copy that arrives gilded) in no purchase statistic; the Blood Price the buyer's purchase (every table) |
| **UR-F9** | LOW | statistics | The Cowboy classifies the natural flavour line, not the printed one (a sign replacement or a skip changes the line). | `gameHistory.ts:451–457` | none — UR-5 · **FIXED rev 9 (UR-5)** — the printed line (the pinned run's applied stage, the legacy resolution unpinned); a $0 run prints none |
| **UR-F10** | — (**closed rev 2: intended**) | money | The Mark's award is minted — no payer, no bank debit — on pinned tables too. Documented as `MINTS_BY_DESIGN`; the exemption's own comment expected S9-1 to close it; S9-1 did not. | P-E; `moneyConservation.test.ts:138`, `:186–254` | OD-UR-4 **decided** (4-A): the mint is intended variant law, not a defect; only the exemption's stale comment remains (Appendix B item 11) *(rev 9: corrected in UR-5)* |
...
| **UR-F12** | LOW | player-facing copy | The gilded chip's tooltip says "Occupies no train-limit slot until this Operating Round ends" — superseded by #1672 (the whole Carcosa lifetime). | `components/TrainBadges.tsx:632` | none — UR-6 (the text follows OD-UR-2's lifetime) · **FIXED rev 10 (UR-6)** — `CARCOSA_CHIP_TOOLTIP`; the chip's `alt` names the train as the panel does |
| **UR-F13** | LOW | Rules Reference | One sentence on the variant (the ±20% / $10 note); the die table, the per-turn aggregate, the Yellow Sign, Carcosa, the fog, the Blood Price and the limit exemption are absent, although S9-3 recorded its rule "for the later Rules Reference pass" and S9-4 makes the Rules Reference the sole player-facing authority. | `RulesReference.tsx:613–614` | OD-UR-8 **decided** (8-B) — UR-6; the Blood Price text waits for OD-UR-5 — *rev 7: OD-UR-5 fully decided; the text can say it (an additional ordinary train; the buyer's marker moves Left 1 / Down 1; the sale names the copy)* · **FIXED rev 10 (UR-6)** — two tagged blocks (Dividends; Buy Trains); the Yellow Sign's trigger and odds stay hidden; no tie direction (UR-F20, UR-7) |
| **UR-F14** | LOW | copy | The variant is "Unpredictable revenue" (`VARIANT_COPY`, Rules Reference, Lobby) and "Unpredictable Routes" (host setup `HostSetupCard.tsx:75`, room list `LobbyRoomList.tsx:51`). | reading | OD-UR-12 **decided** ("Unpredictable Revenue") — UR-6 · **FIXED rev 10 (UR-6)** — the host setup and the room list; the sentence-case labels ("Unpredictable revenue", beside "Gentle rust (easier)") keep their lists' case |
...
*(Rev 9.)* **UR-5 fixed UR-F8 and UR-F9** (and corrected UR-F10's stale comment). Still open: UR-F12, UR-F13, UR-F14
(UR-6); UR-F15 (INFO); UR-F16 (routed to AWS / live multiplayer); UR-F20 (UR-7); and the unpinned (Firestore) residuals
of UR-3 (S10-11). No finding waits on an owner decision.

*(Rev 10.)* **UR-6 fixed UR-F12, UR-F13 and UR-F14** (with the independent UR-4 review's D1 and U-42's copy). Still open:
...
*(Rev 9: as UR-3 and UR-4 did, UR-5 does NOT recompute the matrix. It moves the rows it touches — UR-N57 (UR-F9), UR-N58
(UR-F8) and X11, with UR-N55 re-asserted — to "fixed with durable R / S / RP evidence" ("UR-5 implementation (rev 9)");
their certification status is re-asserted in UR-7, against the constructed game's statistics. The counts stay rev 7's.)*
...
| X14 | a completed Yellow Sign game (S10-21) | S10-21 | none | DEFERRED → UR-7 *(rev 11: **not built in UR-7** — still DEFERRED, routed to UR-8 or a named follow-up; the constructed game restores part of the displaced evidence, not the completed game)* *(rev 12: **CLOSED — SUPERSEDED** by the constructed certification game — UR-8)* | G |
...
*(Rev 12, UR-8: clauses unchanged; interactions and invariants CERTIFIED **25** · CORRECT BUT NEEDS LIVE TEST **2** ·
DEFECT **0** · OWNER DECISION **0** · DEFERRED **0** — X14 CLOSED — SUPERSEDED. The variant is CERTIFIED at
...
routed to UR-8 or a named follow-up.)* *(Rev 12: UR-8 closed S10-21 as SUPERSEDED by G1 / G0; G1 / G0 re-run across the
9 → 10 boundary — identical but for the pin.)*
...
*(Rev 12: not built — **CLOSED — SUPERSEDED** by G1 / G0 for every Unpredictable Revenue assertion; the Bagholder /
Little Engine formats and the long timeline are generic epilogue coverage, recorded on the backlog item — UR-8.)*
...
Next: UR-5, UR-6, UR-7, UR-8, in the owner's order.)* *(Rev 9: **UR-4 is COMPLETE** — owner-gated, committed and pushed
as `a6183b5`. **UR-5 is implemented** (uncommitted, awaiting the owner's full repository Jest gate); it is derived history
...
S10-21's completed game was not built — UR-8 or a named follow-up. Next: UR-8.)* *(Rev 12: **UR-7 is COMPLETE** —
owner-gated, committed and pushed as `ff7a04b`. **UR-8 is done** (uncommitted, awaiting the owner's full-suite gate): the
9 → 10 boundary, items (1) – (8) below carried by row 10. **UR-1 … UR-8 are complete; Unpredictable Revenue is CERTIFIED
at `RULES_ENGINE_VERSION` 10**, subject to the owner's full-suite gate and commit.)*
...
OD-UR-3 — *rev 3: implemented by UR-3*); (5) no gilded Diesel trade-in (UR-4; OD-UR-7 — *rev 3: implemented by UR-3*); (6) the Blood Price's consequences, if OD-UR-5 changes them
...
(UR-3; OD-UR-13). *(Rev 12: all eight are **carried by `RULES_ENGINE_VERSION` 10**, changelog row 10 (1) – (8), UR-8.)*
Named as NOT rules: the award's source (OD-UR-4 keeps the mint — no
...
*(Rev 10: **every item is resolved.** Items 1 – 10 are annotated in place by UR-6 — a bracketed "[UR-6 (audit Appendix B
item N): …]" beside the stale text, which is kept as the record (comment-only; verified). Item 11 was UR-5's. Items 12 –
14 are player-facing copy, changed in UR-6. See "UR-6 implementation (rev 10)".)*
...
11. `utils/moneyConservation.test.ts:186–203` — "If S9-1 is ever closed the second half fails and the exemption … comes
    ...
    comment is wrong, not the exemption; correct it in UR-5.)* *(Rev 9: **corrected in UR-5** — the header and the case's
    two S9-1 sentences now say the mint is variant law (OD-UR-4, D-40); the case's title is kept, because the Batch 7.3 /
    7.5 records cite it by name.)*
12. Player-facing: `components/TrainBadges.tsx:632` (UR-F12). *(Rev 10: **fixed in UR-6** — `CARCOSA_CHIP_TOOLTIP`.)*
13. *(Rev 9, found in UR-5.)* Player-facing: `utils/accolades.ts` — Carcosan Railways' blurb, "Saw the Yellow Sign and never
    paid the Blood Price to be rid of it". Under OD-UR-5(b) the buyer pays the Blood Price, so a Carcosan corporation cannot
    pay "to be rid of it"; the accolade's logic is right (#1421: lived through a stage, never paid), only the words are
    stale. UR-6 (copy). *(Rev 10: **fixed in UR-6** — "Saw the Yellow Sign touch their railroad, and never paid a Blood
    Price to buy a gold-trimmed train." The qualification is unchanged; The Redeemer's blurb already named the buyer.)*
14. *(Rev 9, found in UR-5 — a copy consideration, not a defect.)* Player-facing: `components/EpilogueCharts.tsx` — on an
    Unpredictable Revenue table the Revenue-per-OR chart now shows the revenue paid (its hint: "What each corporation's
    trains ran for", the Activity Log's own "ran for $X" vocabulary for the paid figure), and the fleet ledger's "Runs"
    column counts completed runs (a train the Mark took ran none). UR-6 may say so. *(Rev 10: **done in UR-6** — the
    chart's hint was checked and kept: "ran for" is the Activity Log's word for the paid figure on every table; the ledger's
    column titles now say "The printed value of its completed routes" (Earned) and "Completed runs, counting each train
    separately" (Runs) — true of every game.)*
