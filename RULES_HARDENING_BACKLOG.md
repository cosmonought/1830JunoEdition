...
rulings, rev 6 the owner's clarification of OD-UR-5(a), rev 7 OD-UR-5(b) and the OD-UR-6.1 per-train rule; *rev 8 UR-4's implementation, rev 9 UR-4's completion and UR-5's implementation, rev 10 UR-5's completion and UR-6's implementation*). Verdict **B —
...
(**D-47**); not implemented (UR-F20 → UR-7). *(2026-09-24, documentation only:)* OD-UR-5(a) the Blood Price buyer's
...
| UR-F12 | LOW | the gilded chip's tooltip is stale ("until this Operating Round ends") | ~~UR-6 (text per OD-UR-2's lifetime)~~ **`RESOLVED` — UR-6 (2026-09-25, uncommitted, awaiting the owner's gate)**: `CARCOSA_CHIP_TOOLTIP` — the exemption while gold-trimmed, no Diesel trade-in, the fog at the end of the Operating Round set after the doom trigger, the buyer's Blood Price; `alt` "Gold-trimmed Carcosa train" (`unpredictableRevenueDisclosure`, `batch59`) |
| UR-F13 | LOW | the Rules Reference carries one sentence on the variant | UR-6 — OD-UR-8 decided (8-B); the Blood Price text after OD-UR-5 *(fully decided — D-48, D-50)* **`RESOLVED` — UR-6 (2026-09-25, uncommitted, awaiting the owner's gate)**: two tagged blocks — Dividends (one die per turn on the total, the face table, "rounded to the nearest $10", printed values for route choice, the paid figure; the Yellow Sign's CONSEQUENCES only — the taken train removed from the game for good, half its face value) and Buy Trains (the gold-trimmed train, the fog, the Blood Price paid by the buyer, the cured ordinary train, no phase change, the copy named); the trigger conditions and odds stay hidden (D-42); no tie direction — the tie sentence waits for UR-7 (UR-F20) (`unpredictableRevenueRulesReference`) |
| UR-F14 | LOW | "Unpredictable revenue" vs "Unpredictable Routes" | UR-6 — OD-UR-12 decided ("Unpredictable Revenue") **`RESOLVED` — UR-6 (2026-09-25, uncommitted, awaiting the owner's gate)**: the host setup and the room list say "Unpredictable Revenue"; the sentence-case label lists keep their case ("Unpredictable revenue" beside "Gentle rust (easier)") |
...
no owner-decision blocker) (**COMPLETE 2026-09-25 — owner-gated, committed and pushed as `0d8157e`**; the UR-5 block below) → **UR-6** UI / copy / Rules Reference (**IMPLEMENTED 2026-09-25 — uncommitted, awaiting the owner's gate**; the UR-6 block below) → **UR-7** certification
...
D-47) → **UR-8** the one 9 → 10 boundary and
...
**UR-6 — the players are told what the authority does: UI / copy / Rules Reference parity (2026-09-25 — IMPLEMENTED,
UNCOMMITTED, awaiting the owner's full repository Jest gate).** Replay-neutral presentation: no reducer rule, server
...
S10-11 retires it); two unreachable nits (audit "UR-6 implementation (rev 10)"). **Unpredictable Revenue, and Gentle Rust +
