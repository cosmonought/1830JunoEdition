...
rulings, rev 6 the owner's clarification of OD-UR-5(a), rev 7 OD-UR-5(b) and the OD-UR-6.1 per-train rule). Verdict **B —
...
(**IMPLEMENTED 2026-09-25 — uncommitted, awaiting the owner's gate**; the UR-4 block below)
→ **UR-5** money and statistics (OD-UR-6 fully decided — UR-F8: paid turn totals, printed completed routes per train;
no owner-decision blocker) → **UR-6** UI / copy / Rules Reference → **UR-7** certification
...
**UR-4 — the Blood Price, copy by copy, paid by the buyer (2026-09-25 — IMPLEMENTED, UNCOMMITTED, awaiting the owner's
full repository Jest gate).** OD-UR-5 (D-48, D-50) on every table, pinned and unpinned (the corpus holds no gilded train,
...
boundary. *(Implemented 2026-09-25 — UR-4, uncommitted, awaiting the owner's gate: (a) pinned live with the +1 carried
...
when the train is a Diesel and the award otherwise qualifies (it follows D-48(a)). Not implemented — UR-5 (UR-F8);
...
*(Implemented 2026-09-25 — UR-4, uncommitted, awaiting the owner's gate: UR-F22 `RESOLVED` — the buyer's marker moves, the
...
implemented — UR-5 (UR-F8); derived history only, no version effect. `OWNER DECISION` (statistics). Source: the owner's
