...
rulings, rev 6 the owner's clarification of OD-UR-5(a), rev 7 OD-UR-5(b) and the OD-UR-6.1 per-train rule; *rev 8 UR-4's implementation, rev 9 UR-4's completion and UR-5's implementation*). Verdict **B —
...
| UR-F13 | LOW | the Rules Reference carries one sentence on the variant | UR-6 — OD-UR-8 decided (8-B); the Blood Price text after OD-UR-5 *(fully decided — D-48, D-50)* |
...
no owner-decision blocker) (**IMPLEMENTED 2026-09-25 — uncommitted, awaiting the owner's gate**; the UR-5 block below) → **UR-6** UI / copy / Rules Reference → **UR-7** certification
...
**UR-5 — the statistics of an Unpredictable Revenue game, as ruled (2026-09-25 — IMPLEMENTED, UNCOMMITTED, awaiting the
owner's full repository Jest gate).** OD-UR-6 (D-49, D-51) in `utils/gameHistory.ts` — derived history only: no reducer,
...
Revenue-per-OR chart's hint and the ledger's "Runs" column (audit Appendix B items 13, 14). **Unpredictable Revenue, and
...
the buyer's warning — `RESOLVED` in UR-4 (the two surfaces), with two copy items routed to UR-6.** UR-F21 is **LEGALITY
...
change beyond UR-4's own, no replay effect from the UI, no golden.**
...
(UR-F18). A boundary notice may carry the narration later (UR-6). **Implemented in UR-3 (2026-09-24; committed and pushed as `9d0cf3a`)** —
